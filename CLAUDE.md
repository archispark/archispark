...
`vitest.config.ts` runs two `test.projects`: `web` (jsdom, React components/pages) and `server` (node, `lib/archimate/`+`lib/http/`+`lib/mcp/`+`lib/dashboards/` business logic and Route Handler tests) — add `--project server` or `--project web` to target one only. **Never** put a `*.test.ts` inside `apps/server/pages/api/` — Next.js's Pages Router treats every file there as a live route.
