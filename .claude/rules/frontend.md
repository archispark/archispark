...
# Frontend conventions (`apps/server`, `packages/ui`)

- **Never call `fetch` directly in a component.** Go through the typed
  wrappers in `apps/server/lib/api.ts` (`get`/`post`/`put`/`del` —
  `credentials: "include"`, errors parsed via `err.detail`), then wrap
  those in `apps/server/lib/queries.ts` as `useQuery`/`useMutation` hooks
...
- **Create/edit dialogs use the shared `useFormModal<T>()` hook**
  (`apps/server/hooks/use-form-modal.ts`), which standardizes `open`,
