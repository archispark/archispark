...
mode**, gets full owner-equivalent access to any organization's content
after explicitly entering it (`POST /api/platform/organizations/:id/enter`),
including a suspended one. This bypass is centralized once in
...
Admin acts the same way, but on whichever organization it has entered via
admin mode, always with owner-equivalent access.
