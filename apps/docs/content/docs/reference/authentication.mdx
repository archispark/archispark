...
- A fourth role, **Admin** (Keycloak identifier `platform_admin`), is a
  ...
  organization's content: `POST /api/platform/organizations/:id/enter`
  selects the organization, after which every organization/workspace route
  resolves Admin as a synthetic `owner` — including on a **suspended**
  organization, which blocks every other role. There is no implicit
  fallback: Admin has no membership row anywhere, so nothing is resolved
  until it explicitly enters one, and any stray `organization_members` row
  for that account is ignored either way. `GET /api/platform/organizations/active`
  and `DELETE /api/platform/organizations/active` read and clear which
  organization is currently entered ("exit"). Because admin mode is meant
  to stay dynamic per session rather than frozen, Admin cannot create a
  personal API token (`POST /api/settings/api-tokens` returns `403` for it).
