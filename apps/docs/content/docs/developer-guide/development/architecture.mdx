...
there is deliberately no runtime seed/reseed function or script (the demo
reset cron's `truncateApplicationTables()` wipes them along with every
other table and does not restore them). If `packages/db/seeds/dashboards.sql`
...
`0032_dashboard_system_seed.sql` was produced — `0034_demonstration_datasources_postgres.sql`
is the only such backfill so far, pointing both of `demonstration-datasources`'s
panels at the new Postgres datasource: `elements-postgres` aggregates the
`elements`/`workspaces` tables directly, `fournisseurs-postgres` queries the
dedicated `fournisseurs` demo table. Both dashboard lists
