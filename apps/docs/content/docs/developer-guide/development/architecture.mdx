...
import { Neo4jExportPipelineDiagram } from "@/components/diagrams/neo4j-export-pipeline"
...
organizations (`/api/platform/organizations*`, metadata) and, via **admin
mode**, gets full owner-equivalent access to any organization's content by
default (the smallest existing organization, entered automatically on
login — see `ensureDefaultPlatformOrganization` in
`lib/archimate/platform-context-store.ts` — no explicit
`POST /api/platform/organizations/:id/enter` call required unless switching
to a different one), including a suspended one. This bypass is centralized once in
[`apps/server/lib/archimate/access.ts`](https://github.com/archispark/archispark/blob/main/apps/server/lib/archimate/access.ts)
rather than left to be remembered at every call site; every other role- and
suspension-check there stays unchanged. See
...
Admin acts the same way, but on whichever organization admin mode currently
resolves to (the smallest existing organization by default), always with
...
| Module                    | Role                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `contracts.ts`            | Zod schemas for `DashboardDefinition`, `PanelContent`, `PanelResult`, `PanelVisualizationMetadata`. `ELEMENT_TYPES`/layers reuse `lib/archimate-helpers.ts` (`getLayer`, `ALL_ELEMENT_TYPES`) rather than a separate ArchiMate domain module.                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `repository.ts`           | CRUD over `dashboards`/`dashboard_revisions` (`packages/db/src/schema.ts`), on Drizzle/Postgres like every other table. Every method takes `organizationId`: dashboards are scoped per organization (not global), edits create a new immutable revision, deletes are soft (`deletedAt`).                                                                                                                                                                                                                                                                                                                                                                                                    |
