...
| ------------------------------ | ------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ARCHISPARK_MCP_TOKEN`         | `archimate-vercel` MCP server (`.mcp.json`) | Personal API token — generate from **Mon profil → Tokens API → Nouveau token** on `archispark.cloud` (see [MCP server](../reference/mcp-tools)) |
...
| Server             | Provides                                                                                                                       |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------ |
| `archimate-vercel` | This project's own [MCP server](../reference/mcp-tools), against the production deployment at `archispark.cloud` (`/mcp/`). |
...
- `ARCHISPARK_MCP_TOKEN` — a personal API token for the ArchiSpark MCP
  ...
  `.env.example`, see [MCP server](../reference/mcp-tools)).
