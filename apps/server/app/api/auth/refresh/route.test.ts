...
function makeReq(cookie?: string): NextRequest {
  return new NextRequest("http://localhost:8000/api/auth/refresh", {
    method: "POST",
...
describe("POST /api/auth/refresh", () => {
  ...
  it("returns 401 and clears cookies when KEYCLOAK_CLIENT_ID_WEB is not set", async () => {
  ...
  it("returns 204 and sets fresh cookies on success", async () => {
    ...
    expect(refreshTokens).toHaveBeenCalledWith({ clientId: "archispark-web", refreshToken: "old-rt" });
