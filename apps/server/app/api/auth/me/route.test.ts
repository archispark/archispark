...
vi.mock("@workspace/auth", () => ({
  verifyAnyAccessToken: vi.fn(),
}))

import { verifyAnyAccessToken } from "@workspace/auth"

function makeReq(cookie?: string): NextRequest {
  return new NextRequest("http://localhost:8000/api/auth/me", {
...
describe("GET /api/auth/me", () => {
  afterEach(() => {
    vi.mocked(verifyAnyAccessToken).mockReset()
  })
  ...
  it("returns 401 when the access token is invalid", async () => {
    vi.mocked(verifyAnyAccessToken).mockResolvedValue(null)
    const res = await GET(makeReq("access_token=bad"))
    expect(res.status).toBe(401)
  })

  it("returns the user derived from the token claims", async () => {
    vi.mocked(verifyAnyAccessToken).mockResolvedValue({
