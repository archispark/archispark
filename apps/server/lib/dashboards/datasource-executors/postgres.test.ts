...
describe("assertReadOnly", () => {
  it("accepts a single SELECT statement", () => {
    expect(() =>
      assertReadOnly("SELECT count(*) FROM fournisseurs")
    ).not.toThrow()
  })
  ...
  it("rejects a write statement", () => {
    expect(() => assertReadOnly("DELETE FROM fournisseurs")).toThrow(
      /SELECT unique/
    )
  ...
  it("rejects a write keyword smuggled inside an otherwise SELECT-shaped statement", () => {
    expect(() =>
      assertReadOnly(
        "SELECT * FROM fournisseurs; UPDATE fournisseurs SET actif = false"
      )
    ).toThrow()
  })
})
...
describe("assertOrganizationScoped", () => {
  it("accepts a query referencing $organizationId", () => {
    expect(() =>
      assertOrganizationScoped(
        "SELECT count(*) FROM fournisseurs WHERE organization_id = $organizationId"
      )
    ).not.toThrow()
  })
  ...
  it("rejects a query that doesn't reference $organizationId — the multi-tenant scoping contract", () => {
    expect(() =>
      assertOrganizationScoped("SELECT count(*) FROM fournisseurs")
...
describe("assertPanelQuerySafe", () => {
  it("combines both checks", () => {
    ...
    expect(() =>
      assertPanelQuerySafe("SELECT count(*) FROM fournisseurs")
    ).toThrow(/organizationId/)
    expect(() => assertPanelQuerySafe("DELETE FROM fournisseurs")).toThrow(
      /SELECT unique/
    )
  })
})
...
describe("executePostgresQuery", () => {
  ...
  beforeAll(async () => {
    const [org] = await db
      ...
      .values({
        slug: `postgres-executor-test-${randomUUID()}`,
        name: "Postgres Executor Test Org",
      })
      .returning()
    ...
    const [other] = await db
      .insert(organizations)
      .values({
        slug: `postgres-executor-test-other-${randomUUID()}`,
        name: "Other Org",
      })
      .returning()
    ...
    const [workspace] = await db
      ...
      .values({
        uuid: randomUUID(),
        name: "Postgres Executor Test Workspace",
        organizationId,
        createdById: "test",
      })
      .returning()
  ...
  it("counts only the caller's organization's active rows", async () => {
    const execution = await executePostgresQuery(
      query(
        "SELECT count(*)::integer AS count FROM fournisseurs WHERE organization_id = $organizationId AND actif = $actif"
      ),
      "metrics",
      { actif: true },
      organizationId
  ...
  it("never leaks another organization's rows even if the panel text omitted the filter incorrectly at write time", async () => {
    ...
    await expect(
      executePostgresQuery(
        query("SELECT count(*)::integer AS count FROM fournisseurs"),
        "metrics",
        {},
        organizationId
      )
  ...
  it("rejects a non-SELECT statement even if somehow reached", async () => {
    await expect(
      executePostgresQuery(
        query(
          "DELETE FROM fournisseurs WHERE organization_id = $organizationId"
        ),
        "metrics",
        {},
        organizationId
