...
import "./validation" // ensures extendZodWithOpenApi(z) runs first
import { z } from "zod"
import {
  OpenAPIRegistry,
  OpenApiGeneratorV3,
...
import { ELEMENT_TYPES, RELATIONSHIP_TYPES } from "./schemas"
import {
  ElementCreateSchema,
  ElementUpdateSchema,
...
// /mcp/ lives outside the /api prefix (rewritten to the pages/api/mcp.ts
// transport at the app root — see next.config.ts) — override the server
// for this one operation instead of the /api-prefixed default above.
registry.registerPath({
  method: "post",
  path: "/mcp/",
  tags: ["MCP"],
  ...
  operationId: "mcpPost",
  servers: [
    { url: "https://archispark.cloud", description: "Production" },
    { url: "http://localhost:8000", description: "Serveur local" },
  ],
  description:
    "Point d'entrée JSON-RPC 2.0 du transport streamable-http MCP, en mode " +
    "stateless (pas de session : chaque requête crée un serveur MCP éphémère, " +
    "aucun en-tête `mcp-session-id` à fournir ou attendre). " +
    "Authentification par token API personnel (Bearer), jamais par cookie.",
  request: {
    body: {
      required: true,
  ...
  responses: {
    ...
    401: {
      description: "Token manquant, invalide ou erreur d'authentification",
    },
  },
})
...
const _doc: Record<string, any> = generator.generateDocument({
  ...
  servers: [
    { url: "https://archispark.cloud/api", description: "Production" },
    { url: "http://localhost:8000/api", description: "Serveur local" },
  ],
  tags: [
