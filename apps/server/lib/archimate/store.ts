...
import { serializeToOpenExchange } from "./oxf-serializer"
...
const ARCHIMATE_CATEGORY: Record<string, ArchiCategory> = {
  BusinessActor: "active",
  BusinessRole: "active",
  BusinessCollaboration: "active",
  BusinessInterface: "active",
  ApplicationComponent: "active",
  ApplicationCollaboration: "active",
  ApplicationInterface: "active",
  Node: "active",
  Device: "active",
  SystemSoftware: "active",
  TechnologyCollaboration: "active",
  TechnologyInterface: "active",
  Path: "active",
  CommunicationNetwork: "active",
  Equipment: "active",
  Facility: "active",
...
const STRUCTURAL: ArchiCategory[] = [
  "active",
  "passive",
  "composite",
  "strategy",
...
function isRealizationAllowed(s: ArchiCategory, t: ArchiCategory): boolean {
  ...
  if (
    s === "strategy" &&
    (t === "strategy" || t === "motivation" || t === "behavior")
  )
    return true
...
function isTriggeringOrFlowAllowed(
  relType: string,
  s: ArchiCategory,
  t: ArchiCategory
): boolean {
...
function isRelationshipAllowed(
  relType: string,
  srcType?: string,
  tgtType?: string
): boolean {
  ...
  switch (relType) {
    ...
    case "Composition":
    case "Aggregation":
      return STRUCTURAL.includes(s) && STRUCTURAL.includes(t)
    case "Assignment":
      return isAssignmentAllowed(s, t)
    case "Realization":
      return isRealizationAllowed(s, t)
    case "Serving":
      return (
        (s === "behavior" || s === "active") &&
        (t === "active" || t === "behavior")
    ...
    case "Triggering":
    case "Flow":
      return isTriggeringOrFlowAllowed(relType, s, t)
    case "Access":
      return (
        (s === "behavior" && t === "passive") ||
        (s === "passive" && t === "behavior")
      )
    case "Influence":
      return s === "motivation" || t === "motivation"
    default:
      return false
  }
}
...
async function elementPropsByDbId(
  elemDbIds: number[]
  ...
  const rows = await db
    ...
    .where(inArray(elementProperties.elementId, elemDbIds))
  for (const r of rows) {
...
async function relPropsByDbId(
  ...
  const rows = await db
    ...
    .where(inArray(relationshipProperties.relationshipId, relDbIds))
  for (const r of rows) {
...
async function elementNameMap(
  wsId: number
): Promise<Map<string, ArchiElement>> {
  const rows = await db
    .select()
    .from(elements)
    .where(eq(elements.workspaceId, wsId))
  return new Map(
    rows.map((r) => [
      r.uuid,
      {
...
function rowToElement(
  r: typeof elements.$inferSelect,
...
export async function listElementTypes(wsId: number): Promise<string[]> {
  const rows = await db
    ...
    .from(elements)
    .where(eq(elements.workspaceId, wsId))
  return [...new Set(rows.map((e) => e.type).filter(Boolean))].sort((a, b) =>
    a.localeCompare(b)
  )
}
...
export async function listElements(
  ...
  const out = rows.map((r) =>
    elementOut(rowToElement(r, propsMap.get(r.id) ?? {}))
  )
  return attachResolvedElementImages(out)
}
...
export async function getElementById(
  ...
  const [row] = await db
    .select()
    .from(elements)
    .where(and(eq(elements.workspaceId, wsId), eq(elements.uuid, elementId)))
  if (!row) throw new NotFoundError(`Élément '${elementId}' introuvable.`)
  const propsMap = await elementPropsByDbId([row.id])
  return attachResolvedElementImage(
    elementOut(rowToElement(row, propsMap.get(row.id) ?? {}))
  )
}
...
export async function getElementRelationships(
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        or(
          eq(relationships.sourceUuid, elementId),
          eq(relationships.targetUuid, elementId)
        )
      )
    )
  ...
  return rows.map((r) =>
    relOut(rowToRelationship(r, propsMap.get(r.id) ?? {}, elemMap))
  )
}
...
export async function createElement(
  ...
  const [row] = await db
    ...
    .values({
      ...
      type: input.type,
      name: input.name,
      description: input.documentation ?? null,
    })
    .returning()
  if (!row) throw new Error("Failed to create element")
  const props = await propsIn(input.properties)
  for (const [defUuid, value] of Object.entries(props)) {
    await db
      .insert(elementProperties)
      .values({ elementId: row.id, propertyDefUuid: defUuid, value })
  }
  return attachResolvedElementImage(elementOut(rowToElement(row, props)))
}

export async function updateElement(
  wsId: number,
  elementId: string,
  input: ElementUpdateIn
): Promise<ElementOut> {
  const [row] = await db
    .select()
    .from(elements)
  ...
  if (input.documentation !== undefined)
    patch.description = input.documentation ?? null
  if (Object.keys(patch).length > 0)
    await db.update(elements).set(patch).where(eq(elements.id, row.id))
  if (input.properties !== undefined) {
    await db
      .delete(elementProperties)
      .where(eq(elementProperties.elementId, row.id))
    for (const [defUuid, value] of Object.entries(
      await propsIn(input.properties)
    )) {
      await db
        .insert(elementProperties)
        .values({ elementId: row.id, propertyDefUuid: defUuid, value })
...
export async function listRelationshipTypes(wsId: number): Promise<string[]> {
  const rows = await db
    .select()
    .from(relationships)
    .where(eq(relationships.workspaceId, wsId))
  return [...new Set(rows.map((r) => r.type).filter(Boolean))].sort((a, b) =>
    a.localeCompare(b)
  )
}
...
export async function getRelationshipById(
  ...
  const [row] = await db
    ...
    .where(
      and(
        eq(relationships.workspaceId, wsId),
        eq(relationships.uuid, relationshipId)
      )
    )
...
export async function createRelationship(
  wsId: number,
  input: RelationshipCreateIn
): Promise<RelationshipOut> {
  const [src] = await db
    .select()
    .from(elements)
    .where(and(eq(elements.workspaceId, wsId), eq(elements.uuid, input.source)))
  if (!src)
    throw new ValidationError(`Élément source '${input.source}' introuvable.`)
  const [tgt] = await db
    .select()
    .from(elements)
    .where(and(eq(elements.workspaceId, wsId), eq(elements.uuid, input.target)))
  if (!tgt)
    throw new ValidationError(`Élément cible '${input.target}' introuvable.`)
  const uuid = newId()
  const [row] = await db
    ...
    .values({
      ...
      description: input.documentation ?? null,
      sourceUuid: input.source,
      targetUuid: input.target,
  ...
  for (const [defUuid, value] of Object.entries(props)) {
    await db
      .insert(relationshipProperties)
      .values({ relationshipId: row.id, propertyDefUuid: defUuid, value })
  }
...
export async function updateRelationship(
  ...
  if (input.source !== undefined) {
    const [src] = await db
      ...
      .from(elements)
      .where(
        and(eq(elements.workspaceId, wsId), eq(elements.uuid, input.source))
      )
    if (!src)
      throw new ValidationError(`Élément source '${input.source}' introuvable.`)
  }
  if (input.target !== undefined) {
    const [tgt] = await db
      ...
      .from(elements)
      .where(
        and(eq(elements.workspaceId, wsId), eq(elements.uuid, input.target))
      )
    if (!tgt)
      throw new ValidationError(`Élément cible '${input.target}' introuvable.`)
  }
  ...
  if (input.influence_strength !== undefined)
    patch.influenceModifier = input.influence_strength
  if (Object.keys(patch).length > 0)
    await db
  ...
  if (input.properties !== undefined) {
    await db
      .delete(relationshipProperties)
      .where(eq(relationshipProperties.relationshipId, row.id))
    for (const [defUuid, value] of Object.entries(
      await propsIn(input.properties)
    )) {
      await db
...
export async function listPropertyDefinitions(
  wsId: number
): Promise<PropertyDefinitionOut[]> {
  const rows = await db
    .select()
    .from(propertyDefinitions)
    .where(eq(propertyDefinitions.workspaceId, wsId))
  return rows.map((r) =>
    pdOut({ uuid: r.uuid, name: r.name, type: r.type }, r.isSystem)
  )
}
...
export async function getPropertyDefinitionById(
  ...
  const [row] = await db
    ...
    .where(
      and(
        eq(propertyDefinitions.workspaceId, wsId),
        eq(propertyDefinitions.uuid, id)
      )
    )
  if (!row)
    throw new NotFoundError(`Définition de propriété '${id}' introuvable.`)
  return pdOut({ uuid: row.uuid, name: row.name, type: row.type }, row.isSystem)
}

export async function createPropertyDefinition(
  wsId: number,
  input: PropertyDefinitionCreateIn
): Promise<PropertyDefinitionOut> {
  const uuid = newId()
  const [row] = await db
...
export async function updatePropertyDefinition(
  wsId: number,
  id: string,
  input: PropertyDefinitionUpdateIn
): Promise<PropertyDefinitionOut> {
  const [row] = await db
    .select()
    .from(propertyDefinitions)
  ...
  if (row.isSystem)
    throw new ValidationError(
      "Une définition de propriété système ne peut pas être modifiée."
    )
...
export async function deletePropertyDefinition(
  ...
  const [row] = await db
    .select({
      ...
      isSystem: propertyDefinitions.isSystem,
    })
    .from(propertyDefinitions)
    .where(
      and(
  ...
  if (row.isSystem)
    throw new ValidationError(
      "Une définition de propriété système ne peut pas être supprimée."
    )
  ...
  if (elemIds.length > 0) {
    await db
      ...
      .where(
        and(
          inArray(elementProperties.elementId, elemIds),
          eq(elementProperties.propertyDefUuid, id)
        )
      )
  }
  const relIds = (
  ...
  if (relIds.length > 0) {
    await db
      ...
      .where(
        and(
          inArray(relationshipProperties.relationshipId, relIds),
          eq(relationshipProperties.propertyDefUuid, id)
        )
      )
  }
}
...
async function requireView(
  wsId: number,
  viewId: string
): Promise<typeof views.$inferSelect> {
  const [row] = await db
    .select()
    .from(views)
    .where(and(eq(views.workspaceId, wsId), eq(views.uuid, viewId)))
...
async function computeStatusCounts(
  ...
  const connRows = await db
    .select({
      viewId: connections.viewId,
      relUuid: connections.relationshipUuid,
    })
    .from(connections)
    .where(inArray(connections.viewId, viewIds))

  const relUuids = [
    ...new Set(connRows.map((c) => c.relUuid).filter((u): u is string => !!u)),
  ]
  const relTypeMap = new Map<
    string,
    { type: string; srcUuid: string; tgtUuid: string }
  >()
  if (relUuids.length > 0) {
    const relRows = await db
      .select({
        ...
        srcUuid: relationships.sourceUuid,
        tgtUuid: relationships.targetUuid,
      })
      .from(relationships)
      .where(
        and(
          eq(relationships.workspaceId, wsId),
          inArray(relationships.uuid, relUuids)
        )
      )
    for (const r of relRows)
      relTypeMap.set(r.uuid, {
        type: r.type,
        srcUuid: r.srcUuid,
        tgtUuid: r.tgtUuid,
      })
  ...
  const elementUuids = [
    ...new Set([...relTypeMap.values()].flatMap((r) => [r.srcUuid, r.tgtUuid])),
  ]
  ...
  if (elementUuids.length > 0) {
    const elRows = await db
      ...
      .where(
        and(
          eq(elements.workspaceId, wsId),
          inArray(elements.uuid, elementUuids)
        )
      )
  ...
  for (const c of connRows) {
    ...
    if (
      isRelationshipAllowed(
        rel.type,
        elementTypeMap.get(rel.srcUuid),
        elementTypeMap.get(rel.tgtUuid)
      )
    ) {
      entry.ok += 1
    } else {
...
export async function getRelationshipViews(
  wsId: number,
  relationshipId: string
): Promise<ViewOut[]> {
  const rows = await db
    ...
    .where(
      and(
        eq(views.workspaceId, wsId),
        eq(connections.relationshipUuid, relationshipId)
      )
    )
...
export async function getElementViews(
  wsId: number,
  elementId: string
): Promise<ViewOut[]> {
  const rows = await db
...
async function buildViewOutList(
  wsId: number,
  viewRows: (typeof views.$inferSelect)[]
): Promise<ViewOut[]> {
  if (viewRows.length === 0) return []
  ...
  const [nodeCounts, connCounts, statusMap] = await Promise.all([
    db
      .select({ viewId: nodes.viewId, c: count() })
      .from(nodes)
      .where(inArray(nodes.viewId, viewIds))
      .groupBy(nodes.viewId),
    db
      .select({ viewId: connections.viewId, c: count() })
      .from(connections)
      .where(inArray(connections.viewId, viewIds))
      .groupBy(connections.viewId),
...
export async function updateView(
  ...
  if (Object.keys(patch).length > 0)
    await db.update(views).set(patch).where(eq(views.id, v.id))
...
async function findNode(
  ...
  const [row] = await db
    .select()
    .from(nodes)
    .where(and(eq(nodes.viewId, viewDbId), eq(nodes.uuid, nodeUuid)))
  return row ?? null
}
...
async function loadNodeOut(
  ...
  const nodeRows = await db
    .select()
    .from(nodes)
    .where(eq(nodes.viewId, viewDbId))
  const tree = buildNodeTree(nodeRows, null, new Map())
  const found = findInTree(tree, nodeUuid)
  if (!found)
    throw new NotFoundError(`Nœud '${nodeUuid}' introuvable dans la vue.`)
...
export async function createNode(
  ...
  const [el] = await db
    ...
    .from(elements)
    .where(
      and(eq(elements.workspaceId, wsId), eq(elements.uuid, input.element_id))
    )
  if (!el) throw new NotFoundError(`Élément '${input.element_id}' introuvable.`)
  const [maxRow] = await db
    .select({ c: count() })
    .from(nodes)
    .where(eq(nodes.viewId, v.id))
  const [row] = await db
    .insert(nodes)
    .values({
      viewId: v.id,
...
export async function updateViewNode(
  wsId: number,
  viewId: string,
  nodeUuid: string,
  input: NodeUpdateIn
  ...
  if (input.name !== undefined) patch.name = input.name
  if (Object.keys(patch).length > 0)
    await db.update(nodes).set(patch).where(eq(nodes.id, n.id))
  return loadNodeOut(v.id, nodeUuid)
}

export async function deleteViewNode(
  wsId: number,
  viewId: string,
  nodeUuid: string
): Promise<void> {
  ...
  await db
    .delete(nodes)
    .where(and(eq(nodes.viewId, v.id), eq(nodes.parentNodeUuid, nodeUuid)))
  ...
  await db
    ...
    .where(
      and(
        ...
        or(
          eq(connections.sourceNodeUuid, nodeUuid),
          eq(connections.targetNodeUuid, nodeUuid)
        )
      )
    )
}

function connRowToOut(
  c: typeof connections.$inferSelect,
...
export async function createViewConnection(
  wsId: number,
  viewId: string,
  input: ConnectionCreateIn
): Promise<ConnectionOut> {
  const v = await requireView(wsId, viewId)
  if (!(await findNode(v.id, input.source)))
    throw new ValidationError(`Nœud source '${input.source}' introuvable.`)
  if (!(await findNode(v.id, input.target)))
    throw new ValidationError(`Nœud cible '${input.target}' introuvable.`)
  if (input.relationship_id) {
    const [rel] = await db
      ...
      .where(
        and(
          eq(relationships.workspaceId, wsId),
          eq(relationships.uuid, input.relationship_id)
        )
      )
    if (!rel)
      throw new ValidationError(
        `Relation '${input.relationship_id}' introuvable.`
      )
  }
  const [row] = await db
    .insert(connections)
    .values({
      ...
      sourceNodeUuid: input.source,
      targetNodeUuid: input.target,
    })
    .returning()
...
export async function updateViewConnection(
  wsId: number,
  viewId: string,
  connId: string,
  input: ConnectionUpdateIn
): Promise<ConnectionOut> {
  const v = await requireView(wsId, viewId)
  const [c] = await db
    .select()
    .from(connections)
    .where(and(eq(connections.viewId, v.id), eq(connections.uuid, connId)))
  ...
  if (Object.keys(patch).length > 0)
    await db.update(connections).set(patch).where(eq(connections.id, c.id))
  ...
  return connRowToOut(
    updated!,
    input.source_side ?? null,
    input.target_side ?? null
  )
}

export async function deleteViewConnection(
  wsId: number,
  viewId: string,
  connId: string
): Promise<void> {
...
export async function importModelFromXml(
  wsId: number,
  xml: string
): Promise<ModelInfo> {
  ...
  try {
    ...
    throw new Error(`Erreur de parsing XML : ${(err as Error).message}`, {
      cause: err,
    })
  }
  await modelToDb(wsId, model, isResolvableImageReference)
...
async function propsIn(
  properties: { property_definition_ref: string; value: string }[] | undefined
): Promise<Record<string, string>> {
  const result = Object.fromEntries(
    (properties ?? []).map((p) => [p.property_definition_ref, p.value])
  )
  try {
    assertSystemPropertyValues(result, isResolvableImageReference)
  } catch (error) {
    throw new ValidationError(
