...
export function TokenRevealCard({
  ...
  // MCP is same-origin post-fusion (apps/web + apps/api + apps/mcp-server
  // merged into one deployment) — no more NEXT_PUBLIC_MCP_SERVER_URL build
  // arg pointing at a separate Vercel project, just this origin's /mcp/.
  const mcpUrl =
    ...
    <div className="space-y-3 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 dark:border-amber-800 dark:bg-amber-950/30">
      ...
      <div className="rounded border border-border bg-muted/60 px-3 py-2">
        ...
        <code className="text-[11px] break-all text-foreground">
          {`claude mcp add archimate-vercel ${mcpUrl} --transport http --header "Authorization: Bearer ${created.token}"`}
        </code>
      </div>
    </div>
  )
