...
describe("POST/GET/PUT/DELETE /settings/providers lifecycle", () => {
  ...
  it("creates an OIDC provider (POST)", async () => {
    ...
    expect(res.body.tenant_id).toBe("tenant-abc-123");
  ...
  it("updates client_id, client_secret, issuer_url and tenant_id (PUT)", async () => {
    const res = await request(app).put(`/settings/providers/${providerId}`).send({
      client_id: "client-456",
      client_secret: "secret-def",
      issuer_url: "https://sso.example.com/realms/updated",
      tenant_id: "tenant-updated",
    });
    expect(res.status).toBe(200);
    expect(res.body.client_id).toBe("client-456");
    expect(res.body.issuer_url).toBe("https://sso.example.com/realms/updated");
    expect(res.body.tenant_id).toBe("tenant-updated");
  });
