...
const shared = vi.hoisted(() => {
  ...
  return {
    ...
    promptHandlers,
    resourceHandlers,
    setOnInit: (fn: (id: string) => void) => {
      onInit = fn
    },
    callOnInit: (id: string) => {
      if (onInit) onInit(id)
    },
  }
})
...
vi.mock("api/src/registry.js", () => ({
  getWorkspaces: vi
    .fn()
    .mockResolvedValue([{ id: "1", name: "Default", active: true }]),
  activateWorkspace: vi
    .fn()
    .mockResolvedValue({ id: "2", name: "Other", active: true }),
}))

vi.mock("api/src/access.js", () => ({
  resolveActiveContext: vi
    .fn()
    .mockResolvedValue({ organizationId: 1, workspaceId: 1, orgRole: "owner" }),
  assertWorkspaceAccess: vi
    .fn()
    .mockResolvedValue({ organizationId: 1, workspaceId: 1, orgRole: "owner" }),
}))
...
vi.mock("api/src/store.js", () => ({
  ...
  importModelFromXml: vi
    .fn()
    .mockResolvedValue({ identifier: "m1", name: "Imported" }),
}))
...
vi.mock("@modelcontextprotocol/sdk/server/mcp.js", () => ({
  ...
  McpServer: vi.fn().mockImplementation(function McpServerMock(this: any) {
    ...
    this.connect = vi
      .fn()
      .mockImplementation(async () => shared.callOnInit("test-session-abc"))
  }),
}))
...
vi.mock("@modelcontextprotocol/sdk/server/streamableHttp.js", () => ({
  ...
  StreamableHTTPServerTransport: vi
    .fn()
    .mockImplementation(function TransportMock(this: any, opts: any) {
...
describe("MCP tool: list_elements", () => {
  ...
  it("lists elements with valid type filter", async () => {
    ...
    expect(vi.mocked(listElements)).toHaveBeenCalledWith(
      1,
  ...
  it("throws on invalid element_type", async () => {
    await expect(
      callTool("list_elements", { element_type: "InvalidType" })
...
describe("MCP tool: list_relationships", () => {
  ...
  it("throws on invalid rel_type", async () => {
    await expect(
      callTool("list_relationships", { rel_type: "BadType" })
    ).rejects.toThrow(/invalide/i)
  })
})
...
describe("MCP tool: create_view", () => {
  it("calls createView", async () => {
    ...
    expect(vi.mocked(createView)).toHaveBeenCalledWith(1, {
      name: "My View",
      viewpoint: undefined,
...
describe("MCP tool: create_node", () => {
  it("calls createNode", async () => {
    ...
    expect(vi.mocked(createNode)).toHaveBeenCalledWith(1, "v1", {
      element_id: "e1",
      x: undefined,
      y: undefined,
      w: undefined,
...
describe("MCP tool: create_element", () => {
  ...
  it("throws on invalid type", async () => {
    await expect(
      callTool("create_element", { name: "X", type: "BadType" })
    ).rejects.toThrow(/invalide/i)
  })
})
...
describe("MCP tool: update_element", () => {
  it("updates element name", async () => {
    ...
    expect(vi.mocked(updateElement)).toHaveBeenCalledWith(1, "e1", {
      name: "NewName",
    })
  })
  ...
  it("throws on invalid type", async () => {
    await expect(
      callTool("update_element", { element_id: "e1", type: "BadType" })
    ).rejects.toThrow(/invalide/i)
  })
})
...
describe("MCP tool: create_relationship", () => {
  it("creates relationship with valid type", async () => {
    await callTool("create_relationship", {
      type: "Association",
      source: "e1",
      target: "e2",
    })
...
describe("MCP tool: update_relationship", () => {
  it("updates relationship name", async () => {
    ...
    expect(vi.mocked(updateRelationship)).toHaveBeenCalledWith(1, "r1", {
      name: "NewName",
    })
  })
...
describe("MCP tool: delete_relationship", () => {
  it("deletes relationship and returns confirmation", async () => {
    const result = await callTool("delete_relationship", {
      relationship_id: "r1",
    })
...
describe("MCP tool: create_property_definition", () => {
  it("creates with valid type", async () => {
    await callTool("create_property_definition", {
      name: "Cost",
      type: "number",
    })
  ...
  it("throws on invalid type", async () => {
    await expect(
      callTool("create_property_definition", { name: "X", type: "badtype" })
    ).rejects.toThrow(/invalide/i)
  })

  it("creates without type (defaults to string)", async () => {
    await callTool("create_property_definition", { name: "Note" })
    expect(vi.mocked(createPropertyDefinition)).toHaveBeenCalledWith(1, {
      name: "Note",
      type: undefined,
    })
  })
...
describe("MCP tool: update_property_definition", () => {
  it("updates property definition name", async () => {
    ...
    expect(vi.mocked(updatePropertyDefinition)).toHaveBeenCalledWith(1, "pd1", {
      name: "NewName",
    })
  })
  ...
  it("throws on invalid type", async () => {
    await expect(
      callTool("update_property_definition", { id: "pd1", type: "badtype" })
    ).rejects.toThrow(/invalide/i)
  })
})
...
describe("MCP tool: render_view", () => {
  it("returns SVG by default when view exists", async () => {
    vi.mocked(loadModel).mockResolvedValueOnce(
      modelWithViews([{ uuid: "v1" }]) as never
    )
  ...
  it("throws when view not found", async () => {
    ...
    await expect(
      callTool("render_view", { view_id: "missing" })
...
describe("MCP tool: update_view", () => {
  it("updates view name", async () => {
    const result = await callTool("update_view", {
      view_id: "v1",
      name: "New Name",
    })
    expect(vi.mocked(updateView)).toHaveBeenCalledWith(1, "v1", {
      name: "New Name",
    })
...
describe("MCP tool: update_node", () => {
  it("updates node position", async () => {
    ...
    expect(vi.mocked(updateViewNode)).toHaveBeenCalledWith(1, "v1", "n1", {
      x: 10,
      y: 20,
    })
  })
...
describe("MCP tool: delete_node", () => {
  it("deletes node and returns confirmation", async () => {
    const result = await callTool("delete_node", {
      view_id: "v1",
      node_id: "n1",
    })
...
describe("MCP tool: create_connection", () => {
  it("creates connection between two nodes", async () => {
    ...
    expect(vi.mocked(createViewConnection)).toHaveBeenCalledWith(
      1,
      "v1",
...
describe("MCP tool: delete_connection", () => {
  it("deletes connection and returns confirmation", async () => {
    const result = await callTool("delete_connection", {
      view_id: "v1",
      connection_id: "c1",
    })
...
describe("MCP tool: get_element_relationships", () => {
  it("returns relationships for an element", async () => {
    const result = await callTool("get_element_relationships", {
      element_id: "e1",
    })
...
describe("MCP prompt: create-viewpoint-view", () => {
  it("returns a user message referencing the requested viewpoint", async () => {
    const result = await callPrompt("create-viewpoint-view", {
      viewpoint: "Layered",
    })
    expect(result.messages).toHaveLength(1)
