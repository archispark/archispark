...
const _PROPERTY_DEFINITION_TYPES_STR = [...PROPERTY_DEFINITION_TYPES]
  .sort((a, b) => a.localeCompare(b))
  .join(", ")
const _VIEWPOINTS_STR = [...VIEWPOINTS]
  .sort((a, b) => a.localeCompare(b))
  .join(", ")
...
function createMcpServer(user: TokenUser): McpServer {
  ...
  // gateway as the REST API (access.ts) — honouring this token's pinned
  // organization/workspace scope (Phase 5) rather than a bare workspace id.
  const activeWorkspaceId = (intent: Intent): Promise<number> =>
    resolveActiveContext({ id: user.id, role: user.role }, intent, {
  ...
  mcpServer.registerPrompt(
    "create-viewpoint-view",
    {
      ...
      argsSchema: {
        viewpoint: z
          .string()
          .describe(`Viewpoint ArchiMate cible. Valides: ${_VIEWPOINTS_STR}`),
      },
    },
    async ({ viewpoint }) => ({
      messages: [
  ...
  mcpServer.registerTool(
    ...
    async () =>
      toContent(await store.getModelInfo(await activeWorkspaceId("read")))
  )
  ...
  mcpServer.registerTool(
    ...
    async () => {
      const presentTypes = await store.listElementTypes(
        await activeWorkspaceId("read")
      )
  ...
  mcpServer.registerTool(
    ...
    async ({ element_id }) =>
      toContent(
        await store.getElementById(await activeWorkspaceId("read"), element_id)
      )
  )
  ...
  mcpServer.registerTool(
    ...
    async () => {
      const types = await store.listRelationshipTypes(
        await activeWorkspaceId("read")
      )
  ...
  mcpServer.registerTool(
    ...
    async () => {
      return toContent(await store.listViews(await activeWorkspaceId("read")))
    }
  )
  ...
  mcpServer.registerTool(
    ...
    async ({ view_id }) => {
      return toContent(
        await store.getViewById(await activeWorkspaceId("read"), view_id)
      )
    }
  )
  ...
  mcpServer.registerTool(
    "create_view",
    {
      ...
      inputSchema: {
        ...
        viewpoint: z
          ...
          .optional()
          .nullable()
          .describe(`Point de vue ArchiMate. Valides: ${_VIEWPOINTS_STR}`),
        documentation: z
          .string()
          .optional()
          .nullable()
    ...
    async ({ name, viewpoint, documentation }) => {
      const view = await store.createView(await activeWorkspaceId("write"), {
        name,
        viewpoint,
        documentation,
      })
  ...
  mcpServer.registerTool(
    ...
    async ({ view_id, element_id, x, y, w, h }) => {
      return toContent(
        await store.createNode(await activeWorkspaceId("write"), view_id, {
          element_id,
          x,
          y,
          w,
  ...
  mcpServer.registerTool(
    "update_view",
    {
      ...
      inputSchema: {
        ...
        viewpoint: z
          ...
          .optional()
          .nullable()
          .describe(`Nouveau viewpoint ArchiMate. Valides: ${_VIEWPOINTS_STR}`),
        documentation: z
          .string()
          .optional()
          .nullable()
    ...
    async ({ view_id, name, viewpoint, documentation }) => {
      ...
      return toContent(
        await store.updateView(await activeWorkspaceId("write"), view_id, input)
      )
    }
  )
  ...
  mcpServer.registerTool(
    ...
    async ({ view_id }) => {
      await store.deleteView(await activeWorkspaceId("write"), view_id)
    ...
    "create_connection",
    {
      ...
      inputSchema: {
        ...
        source_side: z
          ...
          .optional()
          .nullable()
          .describe(`Côté du nœud source: ${_EDGE_SIDES_STR}`),
        target_side: z
          .string()
          .optional()
          .nullable()
          .describe(`Côté du nœud cible: ${_EDGE_SIDES_STR}`),
      },
    },
    async ({
      view_id,
  ...
  mcpServer.registerTool(
    ...
    async ({ name, type, documentation, properties }) => {
      ...
      const element = await store.createElement(
        await activeWorkspaceId("write"),
        { name, type, documentation, properties }
      )
  ...
  mcpServer.registerTool(
    ...
    async ({ element_id }) => {
      await store.deleteElement(await activeWorkspaceId("write"), element_id)
  ...
  mcpServer.registerTool(
    ...
    async () =>
      toContent(
        await store.listElementsInViews(await activeWorkspaceId("read"))
      )
  )
  ...
  mcpServer.registerTool(
    ...
    async () => toContent(await getWorkspaces({ id: user.id, role: user.role }))
  )
  ...
  mcpServer.registerTool(
    ...
    async ({ workspace_id }) => {
      return toContent(
        await activateWorkspace({ id: user.id, role: user.role }, workspace_id)
      )
    }
  )
  ...
  mcpServer.registerTool(
    ...
    async () => {
      const xml = await store.exportModelToXml(await activeWorkspaceId("read"))
  ...
  mcpServer.registerTool(
    ...
    async ({ xml }) => {
      return toContent(
        await store.importModelFromXml(await activeWorkspaceId("write"), xml)
      )
    }
  )
  ...
  mcpServer.registerTool(
    ...
    async () =>
      toContent(
        await store.listPropertyDefinitions(await activeWorkspaceId("read"))
      )
  )
  ...
  mcpServer.registerTool(
    ...
    async ({ name, type }) => {
      if (type && !PROPERTY_DEFINITION_TYPES.has(type)) {
        throw new Error(
          `Type invalide: '${type}'. Types valides: ${_PROPERTY_DEFINITION_TYPES_STR}`
        )
      }
      return toContent(
        await store.createPropertyDefinition(await activeWorkspaceId("write"), {
          name,
          type,
        })
      )
  ...
  mcpServer.registerTool(
    ...
    async ({ id }) => {
      await store.deletePropertyDefinition(await activeWorkspaceId("write"), id)
  ...
  mcpServer.registerTool(
    ...
    async ({ view_id }) => {
      const model = await store.loadModel(await activeWorkspaceId("read"))
