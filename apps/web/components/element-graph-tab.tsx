...
function createNode(id: string, label: string, elementType: string, isCentral: boolean, router: ReturnType<typeof useRouter>): Node {
  return {
    id,
    position: { x: 0, y: 0 },
    data: {
...
function expandFrontier(
  ...
  for (let d = 0; d < depth; d++) {
    ...
    for (const rel of eligibleRels) {
      const step = frontierStep(rel, currentFrontier, visited, hiddenElementTypes, byId);
      if (!step) continue;

      visited.add(step.otherId);
      nextFrontier.add(step.otherId);
      if (!nodeMap.has(step.otherId)) {
        nodeMap.set(step.otherId, createNode(step.otherId, step.label, step.elementType, false, router));
      }
    }
...
function computeConflictNodeIds(visibleRels: RelationshipOut[], byId: Map<string, ElementOut>): Set<string> {
...
function buildGraph(
  ...
  nodeMap.set(element.identifier, createNode(element.identifier, element.name, element.type, true, router));

  const eligibleRels = allRelationships.filter((r) => !hiddenRelTypes.has(r.type));
  const visited = expandFrontier(element, eligibleRels, byId, depth, hiddenElementTypes, nodeMap, router);
...
function computeVisitedIds(element: ElementOut, allRelationships: RelationshipOut[], depth: number): Set<string> {
  const visited = new Set<string>([element.identifier]);
  let currentFrontier = new Set<string>([element.identifier]);
