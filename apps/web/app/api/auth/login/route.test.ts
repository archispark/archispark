...
describe("GET /api/auth/login", () => {
  ...
  it("returns 500 when KEYCLOAK_CLIENT_ID_WEB is not set", async () => {
  ...
  it("redirects to Keycloak with PKCE params and sets flow cookies", async () => {
    ...
    const res = await GET(makeReq("/api/auth/login?from=/workspaces"));
    ...
    expect(res.cookies.get("auth_redirect")?.value).toBe("/workspaces");
  });
