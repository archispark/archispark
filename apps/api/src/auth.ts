...
import { verifyAccessToken, type KeycloakClaims, getKeycloakUser, getUserRealmRoles } from "@workspace/auth";
...
export interface AuthRequest extends Request {
  ...
  tokenContext?: { workspaceId: number | null };
}
...
export async function lookupApiToken(token: string): Promise<TokenUser | null> {
  ...
  const result: TokenUser = { id: kcUser.id!, username: kcUser.username, role, workspaceId: row.workspaceId };
    ...
    lookupApiToken(token)
      .then(async (tokenUser) => {
        if (tokenUser) {
          ...
          req.tokenContext = { workspaceId: tokenUser.workspaceId };
          next();
          return;
        }
