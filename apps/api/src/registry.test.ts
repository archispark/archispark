...
describe("POST /workspaces", () => {
  ...
  it("returns 422 for missing xml file path", async () => {
    const res = await request(app)
      .post("/workspaces")
      .send({ name: "XML WS", path: "nonexistent.xml" })
    expect(res.status).toBe(422)
  })
})
...
describe("PUT /workspaces/:id", () => {
  ...
  it("renames a workspace", async () => {
    ...
    const createRes = await request(app)
      .post("/workspaces")
      .send({ name: `ToRename WS ${ts}` })
    const id = createRes.body.id
    const res = await request(app)
      .put(`/workspaces/${id}`)
      .send({ name: `Renamed WS ${ts}` })
    expect(res.status).toBe(200)
  ...
  it("updates the description", async () => {
    ...
    const createRes = await request(app)
      .post("/workspaces")
      .send({ name: `WS Desc ${ts}` })
    const id = createRes.body.id
    const res = await request(app)
      .put(`/workspaces/${id}`)
      .send({ name: `WS Desc ${ts}`, description: "Nouvelle description" })
    expect(res.status).toBe(200)
  ...
  it("clears the description when set to null", async () => {
    ...
    const createRes = await request(app)
      .post("/workspaces")
      .send({ name: `WS Desc Clear ${ts}`, description: "Initiale" })
    const id = createRes.body.id
    const res = await request(app)
      .put(`/workspaces/${id}`)
      .send({ name: `WS Desc Clear ${ts}`, description: null })
    expect(res.status).toBe(200)
...
describe("POST /workspaces/:id/activate", () => {
  it("activates a workspace and marks it active", async () => {
    const createRes = await request(app)
      .post("/workspaces")
      .send({ name: `WS To Activate ${Date.now()}` })
    const id = createRes.body.id
    const res = await request(app).post(`/workspaces/${id}/activate`)
    expect(res.status).toBe(200)
    expect(res.body.active).toBe(true)
    const wsRes = await request(app).get("/workspaces")
    const active = wsRes.body.find(
      (w: { id: string; active: boolean }) => w.id === id
    )
...
describe("DELETE /workspaces/:id", () => {
  it("returns 404 when another user tries to delete a workspace they don't own", async () => {
    const createRes = await request(app)
      .post("/workspaces")
      .send({ name: `Owned WS ${Date.now()}` })
    const id = createRes.body.id
  ...
  it("deletes the active workspace and activates another", async () => {
    await request(app)
      .post("/workspaces")
      .send({ name: "Extra WS for active-delete" })
    const before = (await request(app).get("/workspaces")).body as {
      id: string
      active: boolean
    }[]
    ...
    const after = (await request(app).get("/workspaces")).body as {
      id: string
      active: boolean
    }[]
  ...
  it("returns 404 for unknown workspace id", async () => {
    await request(app)
      .post("/workspaces")
      .send({ name: "Extra WS for unknown delete" })
  ...
  it("allows deleting the last workspace, leaving zero", async () => {
    const list = (await request(app).get("/workspaces")).body as {
      id: string
    }[]
    for (const w of list) {
      expect((await request(app).delete(`/workspaces/${w.id}`)).status).toBe(
        204
      )
    }
