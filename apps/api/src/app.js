...
import { dataSource, recomputeDataSourceTypes } from "./registry.js";
...
function fontOut(obj) {
    ...
    if (name || size !== null || color)
        return { name, size, color };
    return null;
}
function nodeStyleOut(n) {
...
export function connectionOut(c) {
    return {
        identifier: c.uuid,
        name: c.name || null,
...
export function viewOut(v, detail = false) {
    const base = {
        identifier: v.uuid,
        name: v.name || "",
    ...
    if (detail) {
        return { ...base, nodes: v.nodes.map(nodeOut), connections: v.conns.map(connectionOut) };
    }
    return base;
}
// ---------------------------------------------------------------------------
...
export function listElementTypes(ds) {
    return ds.elementTypes;
}
export function listElements(ds, element_type, name) {
    let elements = ds.model.elements;
    if (element_type)
        elements = elements.filter((e) => e.type === element_type);
    if (name) {
...
export function getElementById(ds, element_id) {
    const match = ds.model.elements.find((e) => e.uuid === element_id);
    if (!match)
        throw new Error(`Élément '${element_id}' introuvable.`);
    return elementOut(match);
}
export function listRelationshipTypes(ds) {
    return ds.relationshipTypes;
}
export function listRelationships(ds, rel_type, source_id, target_id) {
    let rels = ds.model.relationships;
    if (rel_type)
        rels = rels.filter((r) => r.type === rel_type);
    if (source_id) {
        rels = rels.filter((r) => {
            const src = r.source;
...
export function getRelationshipById(ds, relationship_id) {
    const match = ds.model.relationships.find((r) => r.uuid === relationship_id);
    if (!match)
        throw new Error(`Relation '${relationship_id}' introuvable.`);
    return relOut(match);
}
export function listViews(ds) {
    return ds.model.views.map((v) => viewOut(v));
}
export function getViewById(ds, view_id) {
    const match = ds.model.views.find((v) => v.uuid === view_id);
    if (!match)
        throw new Error(`Vue '${view_id}' introuvable.`);
    return viewOut(match, true);
}
// ---------------------------------------------------------------------------
// Mutation business logic – Views & Nodes
// ---------------------------------------------------------------------------
export function updateView(ds, view_id, input) {
    ...
    if (input.documentation !== undefined)
        view.desc = input.documentation ?? null;
    if (input.viewpoint !== undefined)
        view.primary_viewpoint = input.viewpoint ?? null;
    return viewOut(view, true);
}
export function deleteView(ds, view_id) {
    const idx = ds.model.views.findIndex((v) => v.uuid === view_id);
    if (idx === -1)
        throw new Error(`Vue '${view_id}' introuvable.`);
    ds.model.views.splice(idx, 1);
}
export function createView(ds, input) {
    const view = {
        uuid: newId(),
        name: input.name,
...
export function createNode(ds, view_id, input) {
    const view = ds.model.views.find((v) => v.uuid === view_id);
    if (!view)
        throw new Error(`Vue '${view_id}' introuvable.`);
    const element = ds.model.elements.find((e) => e.uuid === input.element_id);
    if (!element)
        throw new Error(`Élément '${input.element_id}' introuvable.`);
    const node = {
        uuid: newId(),
        name: null,
        ref: element,
...
// ---------------------------------------------------------------------------
export function createElement(ds, input) {
    const element = {
        uuid: newId(),
        name: input.name,
        type: input.type,
...
export function updateElement(ds, element_id, input) {
    const match = ds.model.elements.find((e) => e.uuid === element_id);
    if (!match)
        throw new Error(`Élément '${element_id}' introuvable.`);
    if (input.name !== undefined)
    ...
    if (input.documentation !== undefined)
        match.desc = input.documentation ?? null;
    if (input.properties !== undefined)
        match.props = propsIn(input.properties);
    recomputeDataSourceTypes(ds);
    return elementOut(match);
}
export function deleteElement(ds, element_id) {
...
// ---------------------------------------------------------------------------
export function createRelationship(ds, input) {
    const srcElem = ds.model.elements.find((e) => e.uuid === input.source);
    const tgtElem = ds.model.elements.find((e) => e.uuid === input.target);
    if (!srcElem)
        throw new Error(`Élément source '${input.source}' introuvable.`);
    if (!tgtElem)
        throw new Error(`Élément cible '${input.target}' introuvable.`);
    const rel = {
        uuid: newId(),
...
export function updateRelationship(ds, relationship_id, input) {
    const match = ds.model.relationships.find((r) => r.uuid === relationship_id);
    if (!match)
        throw new Error(`Relation '${relationship_id}' introuvable.`);
    if (input.name !== undefined)
    ...
    if (input.access_type !== undefined)
        match.access_type = input.access_type;
    if (input.is_directed !== undefined)
        match.is_directed = input.is_directed;
    if (input.influence_strength !== undefined)
        match.influence_strength = input.influence_strength;
    recomputeDataSourceTypes(ds);
    return relOut(match);
}
export function deleteRelationship(ds, relationship_id) {
...
// Elements
app.get("/elements/types", (_req, res) => {
    res.json(listElementTypes(dataSource));
});
app.get("/elements", (req, res) => {
    const type = req.query["type"] || null;
    const name = req.query["name"] || null;
    if (!validateType(type, ELEMENT_TYPES, _ELEMENT_TYPES_STR, "d'élément ArchiMate", res))
        return;
    res.json(listElements(dataSource, type, name));
});
app.get("/elements/:element_id", (req, res) => {
    try {
        res.json(getElementById(dataSource, req.params["element_id"]));
    }
    catch (err) {
...
app.post("/elements", (req, res) => {
    ...
    if (!validateType(body.type, ELEMENT_TYPES, _ELEMENT_TYPES_STR, "d'élément ArchiMate", res))
        return;
    try {
...
app.put("/elements/:element_id", (req, res) => {
    const body = req.body;
    if (body.type !== undefined && !validateType(body.type, ELEMENT_TYPES, _ELEMENT_TYPES_STR, "d'élément ArchiMate", res))
        return;
    try {
        res.json(updateElement(dataSource, req.params["element_id"], body));
    }
    catch (err) {
        res.status(404).json({ detail: err.message });
    }
});
app.delete("/elements/:element_id", (req, res) => {
    try {
        deleteElement(dataSource, req.params["element_id"]);
        res.status(204).send();
    }
    catch (err) {
        res.status(404).json({ detail: err.message });
...
// Relationships
app.get("/relationships/types", (_req, res) => {
...
app.get("/relationships", (req, res) => {
    ...
    if (!validateType(type, RELATIONSHIP_TYPES, _RELATIONSHIP_TYPES_STR, "de relation ArchiMate", res))
        return;
    res.json(listRelationships(dataSource, type, source_id, target_id));
});
app.get("/relationships/:relationship_id", (req, res) => {
    try {
        res.json(getRelationshipById(dataSource, req.params["relationship_id"]));
    }
    catch (err) {
        res.status(404).json({ detail: err.message });
    }
...
app.post("/relationships", (req, res) => {
    ...
    if (!validateType(body.type, RELATIONSHIP_TYPES, _RELATIONSHIP_TYPES_STR, "de relation ArchiMate", res))
        return;
    try {
...
app.put("/relationships/:relationship_id", (req, res) => {
    const body = req.body;
    if (body.type !== undefined && !validateType(body.type, RELATIONSHIP_TYPES, _RELATIONSHIP_TYPES_STR, "de relation ArchiMate", res))
        return;
    try {
        res.json(updateRelationship(dataSource, req.params["relationship_id"], body));
    }
    catch (err) {
...
app.delete("/relationships/:relationship_id", (req, res) => {
    try {
        deleteRelationship(dataSource, req.params["relationship_id"]);
        res.status(204).send();
    }
    catch (err) {
        res.status(404).json({ detail: err.message });
...
app.get("/views/:view_id", (req, res) => {
    try {
        res.json(getViewById(dataSource, req.params["view_id"]));
    }
    catch (err) {
        res.status(404).json({ detail: err.message });
    }
...
app.put("/views/:view_id", (req, res) => {
    const body = req.body;
    try {
        res.json(updateView(dataSource, req.params["view_id"], body));
    }
    catch (err) {
        res.status(404).json({ detail: err.message });
    }
});
app.delete("/views/:view_id", (req, res) => {
    try {
        deleteView(dataSource, req.params["view_id"]);
        res.status(204).send();
    }
    catch (err) {
        res.status(404).json({ detail: err.message });
...
app.post("/views/:view_id/nodes", (req, res) => {
    const body = req.body;
    ...
    try {
        res.status(201).json(createNode(dataSource, req.params["view_id"], body));
    }
    catch (err) {
        res.status(404).json({ detail: err.message });
    }
});
app.get("/views/:view_id/image", async (req, res) => {
...
mcpServer.registerTool("get_model_info", { description: "Retourne les métadonnées globales du modèle ArchiMate chargé (identifiant, nom, version, compteurs).", inputSchema: {} }, async () => toContent(getModelInfo(dataSource)));
mcpServer.registerTool("list_element_types", { description: "Retourne la liste triée des types d'éléments ArchiMate 3.1 présents dans le modèle.", inputSchema: {} }, async () => toContent(listElementTypes(dataSource)));
mcpServer.registerTool("list_elements", {
...
mcpServer.registerTool("get_element", { description: "Retourne le détail d'un élément ArchiMate par son identifiant (champ 'identifier').", inputSchema: { element_id: z.string().describe("Identifiant de l'élément") } }, async ({ element_id }) => toContent(getElementById(dataSource, element_id)));
mcpServer.registerTool("list_relationship_types", { description: "Retourne la liste triée des types de relations ArchiMate 3.1 présents dans le modèle.", inputSchema: {} }, async () => toContent(listRelationshipTypes(dataSource)));
mcpServer.registerTool("list_relationships", {
...
mcpServer.registerTool("get_relationship", { description: "Retourne le détail d'une relation ArchiMate par son identifiant.", inputSchema: { relationship_id: z.string().describe("Identifiant de la relation") } }, async ({ relationship_id }) => toContent(getRelationshipById(dataSource, relationship_id)));
mcpServer.registerTool("list_views", { description: "Liste toutes les vues du modèle avec leur nombre de nœuds et de connexions.", inputSchema: {} }, async () => toContent(listViews(dataSource)));
mcpServer.registerTool("get_view", { description: "Retourne le détail d'une vue ArchiMate par son identifiant.", inputSchema: { view_id: z.string().describe("Identifiant de la vue") } }, async ({ view_id }) => toContent(getViewById(dataSource, view_id)));
mcpServer.registerTool("create_view", {
...
}, async ({ name, viewpoint, documentation }) => toContent(createView(dataSource, { name, viewpoint, documentation })));
mcpServer.registerTool("create_node", {
...
}, async ({ view_id, element_id, x, y, w, h }) => toContent(createNode(dataSource, view_id, { element_id, x, y, w, h })));
// ---------------------------------------------------------------------------
...
mcpServer.registerTool("update_relationship", {
...
mcpServer.registerTool("delete_relationship", {
...
// ---------------------------------------------------------------------------
mcpServer.registerTool("save_model", {
...
mcpServer.registerTool("render_view", {
    description: "Génère une image SVG ou PNG d'une vue ArchiMate. " +
    ...
    const svg = renderViewToSvg(view, dataSource.model);
    return {
        content: [{ type: "image", data: Buffer.from(svg).toString("base64"), mimeType: "image/svg+xml" }],
    };
});
// ---------------------------------------------------------------------------
