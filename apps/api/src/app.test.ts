...
import { resolveActiveContext } from "../src/access.js"

/** TEST_USER_ID's active workspace, resolved the same way the app resolves it for a request. */
async function getActiveWorkspaceId(userId: string): Promise<number> {
  const ctx = await resolveActiveContext({ id: userId, role: "user" }, "read")
...
function makeRelationship(
  overrides: Partial<ArchiRelationship> = {}
): ArchiRelationship {
...
function makeConnection(
  overrides: Partial<ArchiConnection> = {}
): ArchiConnection {
  return {
    uuid: "conn-001",
    name: null,
...
beforeAll(async () => {
  ...
  if (seedEls.length === 0) {
    await request(app)
      .post(`/elements`)
      .send({ name: "App A", type: "ApplicationComponent" })
    await request(app)
      .post(`/elements`)
      .send({ name: "App B", type: "ApplicationComponent" })
    ...
    if (created.length >= 2) {
      await request(app)
        ...
        .send({
          type: "Association",
          source: created[0]!.identifier,
          target: created[1]!.identifier,
        })
    }
  ...
  knownElement =
    elementsData.find((e) => e.identifier && e.type) ?? elementsData[0]!
  knownElementType = knownElement.type
  const name = knownElement.name ?? ""
  knownElementNameFragment = (
  ...
  knownRelationship =
    relationshipsData.find((r) => r.identifier && r.type) ??
    relationshipsData[0]!
...
describe("elementOut helper", () => {
  ...
  it("maps desc to documentation", () => {
    expect(elementOut(makeElement({ desc: "Some doc" })).documentation).toBe(
      "Some doc"
    )
  })
  ...
  it("converts props dict to PropertyOut array", () => {
    const result = elementOut(
      makeElement({ props: { "Capability Level": "0", Status: "active" } })
    )
    expect(result.properties).toHaveLength(2)
    const refs = new Set(
      result.properties.map((p) => p.property_definition_ref)
    )
...
describe("relOut helper", () => {
  ...
  it("maps desc to documentation", () => {
    expect(
      relOut(makeRelationship({ desc: "Relation doc" })).documentation
    ).toBe("Relation doc")
  })

  it("sets access_type for Access relationship", () => {
    const result = relOut(
      makeRelationship({ type: "Access", access_type: "Write" })
    )
  ...
  it("access_type is null for non-Access relationship", () => {
    const result = relOut(
      makeRelationship({ type: "Flow", access_type: "Write" })
    )
  ...
  it("sets is_directed for Association", () => {
    const result = relOut(
      makeRelationship({ type: "Association", is_directed: true })
    )
  ...
  it("is_directed is null for non-Association", () => {
    const result = relOut(
      makeRelationship({ type: "Serving", is_directed: true })
    )
  ...
  it("sets modifier for Influence", () => {
    const result = relOut(
      makeRelationship({ type: "Influence", influence_strength: "+" })
    )
  ...
  it("modifier is null for non-Influence", () => {
    const result = relOut(
      makeRelationship({ type: "Flow", influence_strength: "+" })
    )
...
describe("connectionOut helper", () => {
  ...
  it("maps relationship_ref", () => {
    expect(
      connectionOut(makeConnection({ ref: "rel-abc" })).relationship_ref
    ).toBe("rel-abc")
  })
  ...
  it("style has line_color", () => {
    const result = connectionOut(
      makeConnection({ line_color: { r: 92, g: 92, b: 92 } })
    )
  ...
  it("style is null when no styling", () => {
    expect(
      connectionOut(makeConnection({ line_color: null, line_width: null }))
        .style
    ).toBeNull()
  })
})
...
describe("viewOut helper", () => {
  ...
  it("maps desc to documentation", () => {
    expect(viewOut(makeView({ desc: "View doc" })).documentation).toBe(
      "View doc"
    )
  })

  it("maps primary_viewpoint to viewpoint", () => {
    expect(viewOut(makeView({ primary_viewpoint: "Layered" })).viewpoint).toBe(
      "Layered"
    )
  })
  ...
  it("node_count is correct", () => {
    const nodes = [
      makeNode(),
      makeNode({ uuid: "node-002" }),
      makeNode({ uuid: "node-003" }),
    ]
  ...
  it("connection_count is correct", () => {
    const conns = [
      makeConnection(),
      makeConnection({ uuid: "conn-002" }),
      makeConnection({ uuid: "conn-003" }),
      makeConnection({ uuid: "conn-004" }),
      makeConnection({ uuid: "conn-005" }),
    ]
...
describe("GET /elements/types", () => {
  ...
  it("all types are valid ArchiMate 3.1", async () => {
    ...
    for (const t of data) {
      expect(
        ELEMENT_TYPES.has(t),
        `Type '${t}' not in ArchiMate 3.1 spec`
      ).toBe(true)
    }
  })
})
...
describe("GET /elements", () => {
  ...
  it("returns all elements", async () => {
    ...
    expect(data.length).toBe(
      (await store.listElements(await getActiveWorkspaceId(TEST_USER_ID)))
        .length
    )
  })
  ...
  it("filter by type works", async () => {
    const data = (await request(app).get(`/elements?type=${knownElementType}`))
      .body as ElementOut[]
  ...
  it("filter by type + name returns empty when no match", async () => {
    const data = (
      await request(app).get(`/elements?type=Capability&name=xyznotfound123`)
    ).body
    expect(data).toEqual([])
  })
  ...
  it("filter by name is case-insensitive", async () => {
    ...
    const data = (
      await request(app).get(`/elements?name=${knownElementNameFragment}`)
    ).body as ElementOut[]
    expect(data.length).toBeGreaterThan(0)
    expect(
      data.every((e) =>
        (e.name ?? "").toLowerCase().includes(knownElementNameFragment)
      )
    ).toBe(true)
  })
  ...
  it("combined filter by type and name", async () => {
    ...
    const data = (
      await request(app).get(
        `/elements?type=${knownElementType}&name=${knownElementNameFragment}`
      )
    ).body as ElementOut[]
    expect(data.every((e) => e.type === knownElementType)).toBe(true)
  })
...
describe("GET /elements/:id", () => {
  ...
  it("known id returns correct data", async () => {
    const data = (
      await request(app).get(`/elements/${knownElement.identifier}`)
    ).body
  ...
  it("every id exists in the elements list", async () => {
    const [inViews, elements] = await Promise.all([
      request(app)
        ...
        .then((r) => r.body as string[]),
      request(app)
        .get("/elements")
        .then((r) => r.body as ElementOut[]),
    ])
...
describe("GET /elements/:id/views", () => {
  it("returns 200 for a known element", async () => {
    const res = await request(app).get(
      `/elements/${knownElement.identifier}/views`
    )
    expect(res.status).toBe(200)
  })
  ...
  it("returns an array", async () => {
    const data = (
      await request(app).get(`/elements/${knownElement.identifier}/views`)
    ).body
    expect(Array.isArray(data)).toBe(true)
  })

  it("each result has ViewOut shape", async () => {
    const inViews = (await request(app).get("/elements/in-views"))
      .body as string[]
    if (inViews.length === 0) return
    const id = inViews[0]!
    const data = (await request(app).get(`/elements/${id}/views`))
      .body as ViewOut[]
    expect(data.length).toBeGreaterThan(0)
    for (const v of data) {
  ...
  it("returns empty array for element not in any view", async () => {
    const el = (
      await request(app)
        .post("/elements")
        .send({ type: "BusinessActor", name: "NoViewEl" })
    ).body as ElementOut
    const data = (await request(app).get(`/elements/${el.identifier}/views`))
      .body
    expect(Array.isArray(data)).toBe(true)
  ...
  it("deduplicates views when element appears as multiple nodes in the same view", async () => {
    const el = (
      await request(app)
        .post("/elements")
        .send({ type: "BusinessActor", name: "DedupEl" })
    ).body as ElementOut
    const view = (await request(app).post("/views").send({ name: "DedupView" }))
      .body as ViewOut
    // Add same element twice as two distinct nodes
    await request(app)
      .post(`/views/${view.identifier}/nodes`)
      .send({ element_id: el.identifier, x: 0, y: 0, w: 120, h: 55 })
    await request(app)
      .post(`/views/${view.identifier}/nodes`)
      .send({ element_id: el.identifier, x: 200, y: 0, w: 120, h: 55 })
    const data = (await request(app).get(`/elements/${el.identifier}/views`))
      .body as ViewOut[]
...
describe("GET /elements/:id/relationships", () => {
  it("returns 200 for known element", async () => {
    const res = await request(app).get(
      `/elements/${knownElement.identifier}/relationships`
    )
    expect(res.status).toBe(200)
  })
  ...
  it("each result involves the element as source or target", async () => {
    ...
    ).body as RelationshipOut[]
    for (const r of data) {
      expect(
        r.source === knownElement.identifier ||
          r.target === knownElement.identifier
      ).toBe(true)
    }
  })

  it("returns 401 when unauthenticated", async () => {
    const res = await _request(app).get(
      `/elements/${knownElement.identifier}/relationships`
    )
...
describe("GET /relationships/types", () => {
  ...
  it("is sorted", async () => {
    const data = (await request(app).get(`/relationships/types`))
      .body as string[]
    expect(data).toEqual([...data].sort((a, b) => a.localeCompare(b)))
  })
...
describe("GET /relationships", () => {
  ...
  it("returns all relationships", async () => {
    const data = (await request(app).get(`/relationships`))
      .body as RelationshipOut[]
    expect(data.length).toBe(
      (await store.listRelationships(await getActiveWorkspaceId(TEST_USER_ID)))
        .length
    )
  })
  ...
  it("relationship has required shape", async () => {
    const r = (
      (await request(app).get(`/relationships`)).body as RelationshipOut[]
    )[0]!
  ...
  it("filter by type works", async () => {
    const data = (
      await request(app).get(`/relationships?type=${knownRelationshipType}`)
    ).body as RelationshipOut[]
    expect(data.length).toBeGreaterThan(0)
  ...
  it("invalid type returns 422", async () => {
    expect(
      (await request(app).get(`/relationships?type=NotARelType`)).status
    ).toBe(422)
  })

  it("filter by source_id works", async () => {
    const source = knownRelationship.source
    const data = (await request(app).get(`/relationships?source_id=${source}`))
      .body as RelationshipOut[]
    expect(data.length).toBeGreaterThan(0)
  ...
  it("filter by target_id works", async () => {
    ...
    const data = (await request(app).get(`/relationships?target_id=${target}`))
      .body as RelationshipOut[]
    expect(data.length).toBeGreaterThan(0)
  ...
  it("filter with no match returns empty array", async () => {
    const data = (
      await request(app).get(`/relationships?source_id=${UNKNOWN_ID}`)
    ).body
    expect(data).toEqual([])
  })
  ...
  it("source_name and target_name are present", async () => {
    const data = (await request(app).get(`/relationships`))
      .body as RelationshipOut[]
    expect(data.every((r) => "source_name" in r && "target_name" in r)).toBe(
      true
    )
  })

  it("Access relationships have access_type in ACCESS_TYPES or null", async () => {
    const data = (await request(app).get(`/relationships?type=Access`))
      .body as RelationshipOut[]
    for (const r of data) {
  ...
  it("Association relationships have is_directed field", async () => {
    const data = (await request(app).get(`/relationships?type=Association`))
      .body as RelationshipOut[]
    for (const r of data) {
  ...
  it("Influence relationships have modifier field", async () => {
    const data = (await request(app).get(`/relationships?type=Influence`))
      .body as RelationshipOut[]
    for (const r of data) {
...
describe("GET /relationships/:id", () => {
  it("known id returns 200", async () => {
    expect(
      (await request(app).get(`/relationships/${knownRelationship.identifier}`))
        .status
    ).toBe(200)
  })
  ...
  it("unknown id returns 404", async () => {
    expect(
      (await request(app).get(`/relationships/${UNKNOWN_ID}`)).status
    ).toBe(404)
  })
})
...
describe("GET /views", () => {
  ...
  it("returns all views", async () => {
    ...
    expect(data.length).toBe(
      (await store.listViews(await getActiveWorkspaceId(TEST_USER_ID))).length
    )
  })
  ...
  it("contains known view", async () => {
    const ids = ((await request(app).get(`/views`)).body as ViewOut[]).map(
      (v) => v.identifier
    )
...
describe("GET /views/:id", () => {
  it("known id returns 200", async () => {
    expect(
      (await request(app).get(`/views/${knownView.identifier}`)).status
    ).toBe(200)
  })
  ...
  it("nodes are present and count matches", async () => {
    const data = (await request(app).get(`/views/${knownView.identifier}`))
      .body as ViewDetailOut
  ...
  it("node coordinates are integers", async () => {
    ...
    for (const n of data.nodes) {
      ...
      if (n.w !== null && n.w !== undefined)
        expect(Number.isInteger(n.w)).toBe(true)
    }
  })
  ...
  it("node element_ref references a known element", async () => {
    ...
    const elemIds = new Set(
      ((await request(app).get(`/elements`)).body as ElementOut[]).map(
        (e) => e.identifier
      )
    )
    const refs = data.nodes
      .filter((n) => n.element_ref)
      .map((n) => n.element_ref!)
    if (refs.length > 0) {
...
describe("POST /views", () => {
  ...
  it("returns 201 with ViewDetail shape", async () => {
    const res = await request(app).post(`/views`).send({ name: "Test View" })
  ...
  it("created view appears in GET /views", async () => {
    const viewId = (
      await request(app).post(`/views`).send({ name: "Listed View" })
    ).body.identifier
...
describe("POST /views/:view_id/nodes", () => {
  ...
  beforeAll(async () => {
    const viewRes = await request(app)
      .post(`/views`)
      .send({ name: "Node Test View" })
    testViewId = viewRes.body.identifier
    knownElementId = (
      (await request(app).get(`/elements`)).body as ElementOut[]
    )[0]!.identifier
  })

  afterEach(async () => {
    const detail = (await request(app).get(`/views/${testViewId}`))
      .body as ViewDetailOut
  ...
  it("node appears in GET /views/:id", async () => {
    await request(app)
      .post(`/views/${testViewId}/nodes`)
      .send({ element_id: knownElementId })
    const view = (await request(app).get(`/views/${testViewId}`))
      .body as ViewDetailOut
  ...
  it("returns 404 for unknown element_id", async () => {
    const res = await request(app)
      .post(`/views/${testViewId}/nodes`)
      .send({ element_id: UNKNOWN_ID })
    expect(res.status).toBe(404)
  })

  it("returns 422 when element_id is missing", async () => {
    expect(
      (await request(app).post(`/views/${testViewId}/nodes`).send({})).status
    ).toBe(422)
  })
})
...
describe("Mutation - éléments POST/PUT/DELETE", () => {
  ...
  it("POST /elements sans type retourne 422", async () => {
    const res = await request(app).post(`/elements`).send({ name: "No type" })
    expect(res.status).toBe(422)
  })
  ...
  it("DELETE /elements/:id inconnu retourne 404", async () => {
    expect((await request(app).delete(`/elements/${UNKNOWN_ID}`)).status).toBe(
      404
    )
  })
})
...
describe("Mutation - relations POST/PUT/DELETE", () => {
  ...
  it("DELETE /relationships/:id inconnu retourne 404", async () => {
    expect(
      (await request(app).delete(`/relationships/${UNKNOWN_ID}`)).status
    ).toBe(404)
  })
})
...
function makePropertyDefinition(
  overrides: Partial<ArchiPropertyDefinition> = {}
): ArchiPropertyDefinition {
...
describe("GET /property-definitions", () => {
  ...
  it("items have required shape", async () => {
    const data = (await request(app).get("/property-definitions"))
      .body as PropertyDefinitionOut[]
  ...
  it("count matches model", async () => {
    ...
    expect(data.length).toBe(
      (
        await store.listPropertyDefinitions(
          await getActiveWorkspaceId(TEST_USER_ID)
        )
      ).length
    )
  })
...
describe("POST/GET/PUT/DELETE /property-definitions", () => {
  it("full CRUD lifecycle", async () => {
    ...
    // Confirm gone
    expect((await request(app).get(`/property-definitions/${id}`)).status).toBe(
      404
    )
  })
  ...
  it("GET 404 for unknown id", async () => {
    expect(
      (await request(app).get(`/property-definitions/${UNKNOWN_ID}`)).status
    ).toBe(404)
  })

  it("DELETE 404 for unknown id", async () => {
    expect(
      (await request(app).delete(`/property-definitions/${UNKNOWN_ID}`)).status
    ).toBe(404)
  })
})
...
describe("PUT /views/:view_id", () => {
  ...
  it("returns 404 for unknown id", async () => {
    expect(
      (await request(app).put(`/views/${UNKNOWN_ID}`).send({ name: "X" }))
        .status
    ).toBe(404)
  })
})
...
describe("POST /views/:view_id/nodes", () => {
  ...
  it("creates a node and returns 201", async () => {
    const res = await request(app)
      .post(`/views/${testViewId}/nodes`)
      .send({ element_id: testElemId, x: 10, y: 20, w: 100, h: 60 })
    expect(res.status).toBe(201)
...
describe("PUT /views/:view_id/nodes/:node_id and DELETE", () => {
  ...
  it("returns 404 PUT when view not found", async () => {
    expect(
      (
        await request(app)
          .put(`/views/${UNKNOWN_ID}/nodes/${nodeId}`)
          .send({ x: 0 })
      ).status
    ).toBe(404)
  })
  ...
  it("deletes node via DELETE and returns 204", async () => {
    ...
    expect(
      (await request(app).delete(`/views/${viewId}/nodes/${nId}`)).status
    ).toBe(204)
  })

  it("returns 404 DELETE when view not found", async () => {
    expect(
      (await request(app).delete(`/views/${UNKNOWN_ID}/nodes/${nodeId}`)).status
    ).toBe(404)
  })
})
...
describe("POST/PUT/DELETE /views/:view_id/connections", () => {
  ...
  beforeAll(async () => {
    const vRes = await request(app)
      .post("/views")
      .send({ name: "ConnTestView" })
    viewId = (vRes.body as ViewDetailOut).identifier
    const eRes1 = await request(app)
      .post("/elements")
      .send({ name: "ConnElemA", type: "ApplicationComponent" })
    const eRes2 = await request(app)
      .post("/elements")
      .send({ name: "ConnElemB", type: "ApplicationComponent" })
    const nRes1 = await request(app)
      .post(`/views/${viewId}/nodes`)
      .send({ element_id: eRes1.body.identifier })
    const nRes2 = await request(app)
      .post(`/views/${viewId}/nodes`)
      .send({ element_id: eRes2.body.identifier })
  ...
  it("returns 404 when view not found", async () => {
    expect(
      (
        await request(app)
          .post(`/views/${UNKNOWN_ID}/connections`)
          .send({ source: nodeAId, target: nodeBId })
      ).status
    ).toBe(404)
  })
  ...
  it("returns 404 PUT when connection not found", async () => {
    expect(
      (
        await request(app)
          .put(`/views/${viewId}/connections/${UNKNOWN_ID}`)
          .send({ name: "X" })
      ).status
    ).toBe(404)
  })
  ...
  it("deletes connection and returns 204", async () => {
    ...
    const connId = cRes.body.identifier as string
    expect(
      (await request(app).delete(`/views/${viewId}/connections/${connId}`))
        .status
    ).toBe(204)
  })
  ...
  it("returns 404 DELETE when connection not found", async () => {
    expect(
      (await request(app).delete(`/views/${viewId}/connections/${UNKNOWN_ID}`))
        .status
    ).toBe(404)
  })
})
...
describe("updateViewConnection source/target update", () => {
  it("successfully updates source and target nodes", async () => {
    const vRes = await request(app)
      .post("/views")
      .send({ name: "ConnSrcTgtView" })
    const viewId = (vRes.body as ViewDetailOut).identifier
    const e1 = await request(app)
      .post("/elements")
      .send({ name: "E1", type: "ApplicationComponent" })
    const e2 = await request(app)
      .post("/elements")
      .send({ name: "E2", type: "ApplicationComponent" })
    const e3 = await request(app)
      .post("/elements")
      .send({ name: "E3", type: "ApplicationComponent" })
    const n1 = await request(app)
      .post(`/views/${viewId}/nodes`)
      .send({ element_id: e1.body.identifier })
    const n2 = await request(app)
      .post(`/views/${viewId}/nodes`)
      .send({ element_id: e2.body.identifier })
    const n3 = await request(app)
      .post(`/views/${viewId}/nodes`)
      .send({ element_id: e3.body.identifier })
    const cRes = await request(app)
      .post(`/views/${viewId}/connections`)
      .send({ source: n1.body.identifier, target: n2.body.identifier })
    const connId = cRes.body.identifier as string
    const putRes = await request(app)
      .put(`/views/${viewId}/connections/${connId}`)
