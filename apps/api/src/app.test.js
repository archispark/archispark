...
let elementsData;
let knownElement;
let knownElementType;
let knownElementNameFragment;
let relationshipsData;
let knownRelationship;
let knownRelationshipType;
let knownView;
let sharedMcpSessionId = "";
beforeAll(async () => {
    ds = dataSource;
    const elemRes = await request(app).get(`/elements`);
    elementsData = elemRes.body;
...
describe("viewOut helper", () => {
    ...
    it("detail mode returns ViewDetailOut with nodes and connections", () => {
        const result = viewOut(makeView(), true);
...
describe("GET /elements/types", () => {
    ...
    it("returns array of strings", async () => {
        const data = (await request(app).get(`/elements/types`)).body;
...
describe("GET /elements", () => {
    ...
    it("returns all elements", async () => {
        const data = (await request(app).get(`/elements`)).body;
    ...
    it("filter by type works", async () => {
        const data = (await request(app).get(`/elements?type=${knownElementType}`)).body;
    ...
    it("filter by name is case-insensitive", async () => {
        if (!knownElementNameFragment)
            return;
        const data = (await request(app).get(`/elements?name=${knownElementNameFragment}`)).body;
        expect(data.length).toBeGreaterThan(0);
    ...
    it("combined filter by type and name", async () => {
        if (!knownElementNameFragment)
            return;
        const data = (await request(app).get(`/elements?type=${knownElementType}&name=${knownElementNameFragment}`)).body;
        expect(data.every((e) => e.type === knownElementType)).toBe(true);
    });
});
// ===========================================================================
...
describe("GET /relationships/types", () => {
    ...
    it("is sorted", async () => {
        const data = (await request(app).get(`/relationships/types`)).body;
        expect(data).toEqual([...data].sort());
    });
...
describe("GET /relationships", () => {
    ...
    it("returns all relationships", async () => {
        const data = (await request(app).get(`/relationships`)).body;
        expect(data.length).toBe(ds.model.relationships.length);
    });
    it("relationship has required shape", async () => {
        const r = (await request(app).get(`/relationships`)).body[0];
    ...
    it("filter by type works", async () => {
        const data = (await request(app).get(`/relationships?type=${knownRelationshipType}`)).body;
        expect(data.length).toBeGreaterThan(0);
    ...
    it("filter by source_id works", async () => {
        ...
        const data = (await request(app).get(`/relationships?source_id=${source}`)).body;
        expect(data.length).toBeGreaterThan(0);
        expect(data.every((r) => r.source === source)).toBe(true);
    });
    it("filter by target_id works", async () => {
        const target = knownRelationship.target;
        const data = (await request(app).get(`/relationships?target_id=${target}`)).body;
        expect(data.length).toBeGreaterThan(0);
    ...
    it("Access relationships have access_type in ACCESS_TYPES or null", async () => {
        const data = (await request(app).get(`/relationships?type=Access`)).body;
        for (const r of data) {
    ...
    it("Association relationships have is_directed field", async () => {
        const data = (await request(app).get(`/relationships?type=Association`)).body;
        for (const r of data) {
            expect("is_directed" in r).toBe(true);
        }
    });
    it("Influence relationships have modifier field", async () => {
        const data = (await request(app).get(`/relationships?type=Influence`)).body;
        for (const r of data) {
...
describe("GET /views", () => {
    ...
    it("returns all views", async () => {
        const data = (await request(app).get(`/views`)).body;
        expect(data.length).toBe(ds.model.views.length);
    });
    it("view has required shape", async () => {
        const v = (await request(app).get(`/views`)).body[0];
    ...
    it("contains known view", async () => {
        const ids = (await request(app).get(`/views`)).body.map((v) => v.identifier);
...
describe("GET /views/:id", () => {
    ...
    it("connection relationship_ref references a known relationship", async () => {
        const data = (await request(app).get(`/views/${knownView.identifier}`)).body;
        const relIds = new Set((await request(app).get(`/relationships`)).body.map((r) => r.identifier));
        for (const c of data.connections) {
            if (c.relationship_ref)
                expect(relIds.has(c.relationship_ref)).toBe(true);
        }
    });
    it("node element_ref references a known element", async () => {
        const data = (await request(app).get(`/views/${knownView.identifier}`)).body;
        const elemIds = new Set((await request(app).get(`/elements`)).body.map((e) => e.identifier));
...
describe("POST /views", () => {
    ...
    afterEach(async () => {
        for (const id of createdViewIds.splice(0)) {
            ...
            if (idx !== -1)
                dataSource.model.views.splice(idx, 1);
        }
    });
    ...
    it("created view appears in GET /views", async () => {
        ...
        const list = (await request(app).get(`/views`)).body;
...
describe("POST /views/:view_id/nodes", () => {
    ...
    beforeAll(async () => {
        ...
        knownElementId = (await request(app).get(`/elements`)).body[0].identifier;
    });
    afterEach(async () => {
    ...
    it("node appears in GET /views/:id", async () => {
        ...
        const view = (await request(app).get(`/views/${testViewId}`)).body;
...
describe("Mutation - éléments POST/PUT/DELETE", () => {
    ...
    it("POST /elements crée un élément (201)", async () => {
        ...
        createdElementId = res.body.identifier;
    });
...
describe("Mutation - relations POST/PUT/DELETE", () => {
    ...
    it("POST /relationships crée une relation (201)", async () => {
        ...
        createdRelId = res.body.identifier;
    });
...
describe("MCP service", () => {
    ...
    beforeAll(async () => {
        ...
        sharedMcpSessionId = initRes.headers["mcp-session-id"];
    }, 15000);
    ...
    it("initialize and list tools via JSON-RPC", async () => {
        ...
        const registeredTools = new Set(toolsPayload.result.tools.map((t) => t.name));
...
describe("MCP tool calls via JSON-RPC", () => {
    ...
    it("list_element_types returns sorted types array", async () => {
        ...
        const types = parseToolResult(payload);
    ...
    it("list_elements with valid element_type filter", async () => {
        ...
        const elems = parseToolResult(payload);
    ...
    it("list_relationships with valid rel_type filter", async () => {
        ...
        const rels = parseToolResult(payload);
    ...
    it("list_views returns view list", async () => {
        ...
        const views = parseToolResult(payload);
    ...
    it("create_element (valid type), update_element (all fields), delete_element", async () => {
        ...
        const elemId = created["identifier"];
    ...
    it("create_relationship (all fields), update_relationship (all fields), delete_relationship", async () => {
        ...
        const e1Id = e1["identifier"];
        const e2Id = e2["identifier"];
...
describe("renderViewToSvg – with nodes", () => {
    ...
    it("renders junctions as circles (no element rect)", () => {
        const junction = { uuid: "j1", name: "", type: "AndJunction", desc: null, props: {} };
    ...
    it("renders Grouping with dashed border", () => {
        const grouping = { uuid: "g1", name: "My Group", type: "Grouping", desc: null, props: {} };
...
describe("renderViewToSvg – connection features", () => {
    ...
    it("sorts Grouping nodes before other nodes", () => {
        const groupElem = { uuid: "g1", name: "Group", type: "Grouping", desc: null, props: {} };
        const m = { ...baseModel, elements: [e1, groupElem] };
