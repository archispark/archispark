...
describe("store – elements", () => {
  ...
  it("filters list by type and name", async () => {
    ...
    await store.createElement(wsId, {
      name: "Beta",
      type: "ApplicationComponent",
    })
  ...
  it("listElementTypes returns sorted distinct types", async () => {
    ...
    expect(await store.listElementTypes(wsId)).toEqual([
  ...
  it("getElementById throws for unknown id", async () => {
    await expect(store.getElementById(wsId, "nope")).rejects.toThrow(
      /introuvable/
    )
  })

  it("updates fields, properties, and leaves omitted fields untouched", async () => {
    const e = await store.createElement(wsId, {
      name: "Old",
      type: "Goal",
      documentation: "keep",
    })
    await store.updateElement(wsId, e.identifier, {
      name: "New",
      type: "BusinessActor",
    })
  ...
  it("updateElement throws for unknown id", async () => {
    await expect(
      store.updateElement(wsId, "nope", { name: "x" })
  ...
  it("deletes an element and cascades its relationships and view nodes", async () => {
    ...
    await store.createRelationship(wsId, {
      type: "Association",
      source: a.identifier,
      target: b.identifier,
    })
    ...
    expect((await store.getViewById(wsId, view.identifier)).nodes).toHaveLength(
      0
    )
  })

  it("deleteElement throws for unknown id", async () => {
    await expect(store.deleteElement(wsId, "nope")).rejects.toThrow(
      /introuvable/
    )
  })
...
describe("store – relationships", () => {
  async function twoElements() {
    const a = await store.createElement(wsId, { name: "A", type: "Goal" })
    const b = await store.createElement(wsId, {
      name: "B",
      type: "ApplicationComponent",
    })
    return [a, b] as const
  ...
  it("lists and filters relationships, and lists types", async () => {
    ...
    expect(
      await store.listRelationships(wsId, null, a.identifier)
    ).toHaveLength(2)
    expect(
      await store.listRelationships(wsId, null, null, b.identifier)
    ).toHaveLength(2)
    expect(await store.listRelationshipTypes(wsId)).toEqual([
      "Association",
      "Composition",
    ])
  })
  ...
  it("getElementRelationships returns relationships touching an element", async () => {
    ...
    expect(
      await store.getElementRelationships(wsId, a.identifier)
    ).toHaveLength(1)
    expect(
      await store.getElementRelationships(wsId, b.identifier)
    ).toHaveLength(1)
  })
  ...
  it("updates name/source/target with validation, and deletes", async () => {
    ...
    await expect(
      store.updateRelationship(wsId, rel.identifier, { source: "nope" })
    ).rejects.toThrow(/source/)
    await expect(
      store.updateRelationship(wsId, rel.identifier, { target: "nope" })
    ).rejects.toThrow(/cible/)
  ...
  it("throws for unknown relationship id (get/update/delete)", async () => {
    await expect(store.getRelationshipById(wsId, "nope")).rejects.toThrow(
      /introuvable/
    )
    await expect(store.updateRelationship(wsId, "nope", {})).rejects.toThrow(
      /introuvable/
    )
    await expect(store.deleteRelationship(wsId, "nope")).rejects.toThrow(
      /introuvable/
    )
  })
})
...
describe("store – property definitions", () => {
  it("CRUD lifecycle with type defaulting", async () => {
    ...
    const updName = await store.updatePropertyDefinition(wsId, pd.identifier, {
      name: "Phase",
    })
    expect(updName.name).toBe("Phase")
    expect(updName.type).toBe("string")
    const updType = await store.updatePropertyDefinition(wsId, pd.identifier, {
      type: "number",
    })
  ...
  it("delete cascades property values on elements and relationships", async () => {
    ...
    const a = await store.createElement(wsId, {
      name: "A",
      type: "Goal",
      properties: [
        { property_definition_ref: pd.identifier, value: "x" },
        { property_definition_ref: "keep", value: "y" },
      ],
    })
    const b = await store.createElement(wsId, { name: "B", type: "Goal" })
    const rel = await store.createRelationship(wsId, {
      ...
      source: a.identifier,
      target: b.identifier,
      properties: [{ property_definition_ref: pd.identifier, value: "z" }],
    })

    await store.deletePropertyDefinition(wsId, pd.identifier)
    const elem = await store.getElementById(wsId, a.identifier)
    expect(
      elem.properties.find((p) => p.property_definition_ref === pd.identifier)
    ).toBeUndefined()
    expect(
      elem.properties.find((p) => p.property_definition_ref === "keep")
    ).toBeTruthy()
    const r = await store.getRelationshipById(wsId, rel.identifier)
    expect(
      r.properties.find((p) => p.property_definition_ref === pd.identifier)
    ).toBeUndefined()
  })

  it("throws for unknown id (get/update/delete)", async () => {
    await expect(store.getPropertyDefinitionById(wsId, "nope")).rejects.toThrow(
      /introuvable/
    )
    await expect(
      store.updatePropertyDefinition(wsId, "nope", { name: "x" })
    ).rejects.toThrow(/introuvable/)
    await expect(store.deletePropertyDefinition(wsId, "nope")).rejects.toThrow(
      /introuvable/
    )
  })
})
...
describe("store – views, nodes, connections", () => {
  it("creates/updates/lists/deletes views", async () => {
    const v = await store.createView(wsId, {
      name: "V",
      viewpoint: "Layered",
      documentation: "doc",
    })
    ...
    const upd = await store.updateView(wsId, v.identifier, {
      name: "Renamed",
      documentation: null,
      viewpoint: null,
    })
  ...
  it("view operations throw for unknown view", async () => {
    ...
    await expect(store.updateView(wsId, "nope", { name: "x" })).rejects.toThrow(
      /introuvable/
    )
  ...
  it("creates nodes with coordinates and validates view + element", async () => {
    ...
    const node = await store.createNode(wsId, v.identifier, {
      element_id: a.identifier,
      x: 10,
      y: 20,
      w: 120,
      h: 55,
    ...
    await expect(
      store.createNode(wsId, "no-view", { element_id: a.identifier })
    ).rejects.toThrow(/introuvable/)
    await expect(
      store.createNode(wsId, v.identifier, { element_id: "no-elem" })
    ).rejects.toThrow(/introuvable/)
  })
  ...
  it("updates and deletes nodes (with connection cleanup)", async () => {
    const a = await store.createElement(wsId, { name: "A", type: "Goal" })
    const v = await store.createView(wsId, { name: "V" })
    const n1 = await store.createNode(wsId, v.identifier, {
      element_id: a.identifier,
    })
    const n2 = await store.createNode(wsId, v.identifier, {
      element_id: a.identifier,
    })
    const upd = await store.updateViewNode(wsId, v.identifier, n1.identifier, {
      x: 5,
      name: "renamed",
    })
    expect(upd.x).toBe(5)
    ...
    await store.createViewConnection(wsId, v.identifier, {
      source: n1.identifier,
    ...
    await expect(
      store.updateViewNode(wsId, v.identifier, "nope", {})
    ).rejects.toThrow(/introuvable/)
    await expect(
      store.deleteViewNode(wsId, v.identifier, "nope")
    ).rejects.toThrow(/introuvable/)
  })
  ...
  it("creates/updates/deletes connections with validation", async () => {
    ...
    const n2 = await store.createNode(wsId, v.identifier, {
      element_id: b.identifier,
    })

    const conn = await store.createViewConnection(wsId, v.identifier, {
      source: n1.identifier,
      target: n2.identifier,
    ...
    await expect(
      store.createViewConnection(wsId, v.identifier, {
        source: "nope",
        target: n2.identifier,
      })
    ).rejects.toThrow(/source/)
    ...
    const upd = await store.updateViewConnection(
      wsId,
      v.identifier,
      conn.identifier,
      { name: "c2", source: n2.identifier, target: n1.identifier }
    )
    expect(upd.name).toBe("c2")
    expect(upd.source).toBe(n2.identifier)
    await expect(
      store.updateViewConnection(wsId, v.identifier, conn.identifier, {
        source: "nope",
      })
    ).rejects.toThrow(/source/)
    await expect(
    ...
    await expect(
      store.updateViewConnection(wsId, v.identifier, "nope", {})
    ).rejects.toThrow(/introuvable/)

    await store.deleteViewConnection(wsId, v.identifier, conn.identifier)
    expect(
      (await store.getViewById(wsId, v.identifier)).connections
    ).toHaveLength(0)
    await expect(
      store.deleteViewConnection(wsId, v.identifier, "nope")
    ).rejects.toThrow(/introuvable/)
  })
})
...
describe("store – XML export/import", () => {
  ...
  it("importModelFromXml throws on invalid XML", async () => {
    await expect(store.importModelFromXml(wsId, "not-xml")).rejects.toThrow(
      /parsing XML/
    )
  })
})
...
describe("store – view conflict counting", () => {
  ...
  it("isRelationshipAllowed covers Composition, Realization, Triggering via listViews", async () => {
    ...
    const comp1 = await store.createElement(wsId, {
      name: "C1",
      type: "ApplicationComponent",
    })
    const comp2 = await store.createElement(wsId, {
      name: "C2",
      type: "ApplicationComponent",
    })
    const relComp = await store.createRelationship(wsId, {
      type: "Composition",
      source: comp1.identifier,
      target: comp2.identifier,
    })
    // Realization: behavior→passive → ok
    const beh = await store.createElement(wsId, {
      name: "Svc",
      type: "ApplicationService",
    })
    const pas = await store.createElement(wsId, {
      name: "Obj",
      type: "DataObject",
    })
    const relReal = await store.createRelationship(wsId, {
      type: "Realization",
      source: beh.identifier,
      target: pas.identifier,
    })
    // Triggering: behavior→behavior → ok
    const beh2 = await store.createElement(wsId, {
      name: "Func",
      type: "ApplicationFunction",
    })
    const relTrig = await store.createRelationship(wsId, {
      type: "Triggering",
      source: beh.identifier,
      target: beh2.identifier,
    })

    const v = await store.createView(wsId, { name: "V3" })
    const nC1 = await store.createNode(wsId, v.identifier, {
      element_id: comp1.identifier,
    })
    const nC2 = await store.createNode(wsId, v.identifier, {
      element_id: comp2.identifier,
    })
    const nBeh = await store.createNode(wsId, v.identifier, {
      element_id: beh.identifier,
    })
    const nPas = await store.createNode(wsId, v.identifier, {
      element_id: pas.identifier,
    })
    const nBeh2 = await store.createNode(wsId, v.identifier, {
      element_id: beh2.identifier,
    })

    await store.createViewConnection(wsId, v.identifier, {
      source: nC1.identifier,
      target: nC2.identifier,
      relationship_id: relComp.identifier,
    })
    await store.createViewConnection(wsId, v.identifier, {
      source: nBeh.identifier,
      target: nPas.identifier,
      relationship_id: relReal.identifier,
    })
    await store.createViewConnection(wsId, v.identifier, {
      source: nBeh.identifier,
      target: nBeh2.identifier,
      relationship_id: relTrig.identifier,
    })
  ...
  it("isRelationshipAllowed covers Assignment and Realization false-branches via listViews", async () => {
    ...
    const relAssign = await store.createRelationship(wsId, {
      type: "Assignment",
      source: goal1.identifier,
      target: goal2.identifier,
    })
    ...
    const nG1 = await store.createNode(wsId, v.identifier, {
      element_id: goal1.identifier,
    })
    const nG2 = await store.createNode(wsId, v.identifier, {
      element_id: goal2.identifier,
    })
    const nC1 = await store.createNode(wsId, v.identifier, {
      element_id: comp1.identifier,
    })
    ...
    await store.createViewConnection(wsId, v.identifier, {
      source: nG1.identifier,
      target: nG2.identifier,
      relationship_id: relAssign.identifier,
    })
    await store.createViewConnection(wsId, v.identifier, {
      source: nC1.identifier,
      target: nC2.identifier,
  ...
  it("isRelationshipAllowed covers Access, Influence, Flow via listViews", async () => {
    ...
    const svc = await store.createElement(wsId, {
      name: "Svc",
      type: "ApplicationService",
    })
    const obj = await store.createElement(wsId, {
      name: "Obj",
      type: "DataObject",
    })
    const relAccess = await store.createRelationship(wsId, {
      type: "Access",
      source: svc.identifier,
      target: obj.identifier,
    })
    // Influence: motivation→anything → ok
    const goal = await store.createElement(wsId, { name: "Goal", type: "Goal" })
    const comp = await store.createElement(wsId, {
      name: "Comp",
      type: "ApplicationComponent",
    })
    const relInfl = await store.createRelationship(wsId, {
      type: "Influence",
      source: goal.identifier,
      target: comp.identifier,
    })
    // Flow: behavior→behavior → ok (tests Flow branch at line 103-104)
    const func1 = await store.createElement(wsId, {
      name: "F1",
      type: "ApplicationFunction",
    })
    const func2 = await store.createElement(wsId, {
      name: "F2",
      type: "ApplicationFunction",
    })
    const relFlow = await store.createRelationship(wsId, {
      type: "Flow",
      source: func1.identifier,
      target: func2.identifier,
    })

    const v = await store.createView(wsId, { name: "V4" })
    const nSvc = await store.createNode(wsId, v.identifier, {
      element_id: svc.identifier,
    })
    const nObj = await store.createNode(wsId, v.identifier, {
      element_id: obj.identifier,
    })
    const nGoal = await store.createNode(wsId, v.identifier, {
      element_id: goal.identifier,
    })
    const nComp = await store.createNode(wsId, v.identifier, {
      element_id: comp.identifier,
    })
    const nF1 = await store.createNode(wsId, v.identifier, {
      element_id: func1.identifier,
    })
    const nF2 = await store.createNode(wsId, v.identifier, {
      element_id: func2.identifier,
    })

    await store.createViewConnection(wsId, v.identifier, {
      source: nSvc.identifier,
      target: nObj.identifier,
      relationship_id: relAccess.identifier,
    })
    await store.createViewConnection(wsId, v.identifier, {
      source: nGoal.identifier,
      target: nComp.identifier,
      relationship_id: relInfl.identifier,
    })
    await store.createViewConnection(wsId, v.identifier, {
      source: nF1.identifier,
      target: nF2.identifier,
      relationship_id: relFlow.identifier,
    })
...
describe("store – relationship optional fields", () => {
  it("updateRelationship with access_type, is_directed, influence_strength and properties", async () => {
    ...
    const pd = await store.createPropertyDefinition(wsId, {
      name: "status",
      type: "string",
    })
    const rel = await store.createRelationship(wsId, {
