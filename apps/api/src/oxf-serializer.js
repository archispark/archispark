...
function langText(text, indent, tag) {
...
function renderElement(e, indent) {
    const attrs = `identifier="${esc(e.uuid)}" xsi:type="${esc(e.type)}"`;
    const inner = [];
    if (e.name)
        inner.push(langText(e.name, `${indent}  `, "name"));
    if (e.desc)
        inner.push(`${indent}  <documentation xml:lang="fr">${esc(e.desc)}</documentation>`);
...
function renderRelationship(r, indent) {
    ...
    if (r.type === "Access" && r.access_type)
        attrParts.push(`accessType="${esc(r.access_type)}"`);
    if (r.type === "Association" && r.is_directed != null)
        attrParts.push(`isDirected="${r.is_directed}"`);
    if (r.type === "Influence" && r.influence_strength)
        attrParts.push(`modifier="${esc(r.influence_strength)}"`);
    const inner = [];
    if (r.name)
        inner.push(langText(r.name, `${indent}  `, "name"));
    if (r.desc)
        inner.push(`${indent}  <documentation xml:lang="fr">${esc(r.desc)}</documentation>`);
...
function renderStyle(fill, line, fontName, fontSize, fontColor, lineWidth, indent) {
    const hasAny = fill || line || fontName || fontSize !== null || fontColor || lineWidth !== null;
    if (!hasAny)
        return "";
    const inner = [];
    if (line)
        inner.push(`${indent}  <lineColor${colorAttrs(line)} />`);
    if (fill)
        inner.push(`${indent}  <fillColor${colorAttrs(fill)} />`);
    if (fontName || fontSize !== null || fontColor) {
        const fa = [];
        if (fontName)
            fa.push(`name="${esc(fontName)}"`);
        if (fontSize !== null)
            fa.push(`size="${fontSize}"`);
...
function renderNode(n, indent) {
    ...
    const attrParts = [`identifier="${esc(n.uuid)}"`];
    if (elementRef)
        attrParts.push(`elementRef="${esc(elementRef)}"`);
    attrParts.push(`xsi:type="${xsiType}"`);
    if (n.x != null)
        attrParts.push(`x="${Math.round(n.x)}"`);
    if (n.y != null)
        attrParts.push(`y="${Math.round(n.y)}"`);
    if (n.w != null)
        attrParts.push(`w="${Math.round(n.w)}"`);
    if (n.h != null)
        attrParts.push(`h="${Math.round(n.h)}"`);
    const inner = [];
    if (n.name && !elementRef)
        inner.push(langText(n.name, `${indent}  `, "label"));
    ...
    for (const child of n.nodes)
        inner.push(renderNode(child, `${indent}  `));
    if (inner.length === 0)
        return `${indent}<node ${attrParts.join(" ")} />`;
    return `${indent}<node ${attrParts.join(" ")}>\n${inner.join("\n")}\n${indent}</node>`;
}
function renderConnection(c, indent) {
    const xsiType = c.ref ? "Relationship" : "Line";
    const attrParts = [`identifier="${esc(c.uuid)}"`];
    if (c.ref)
        attrParts.push(`relationshipRef="${esc(c.ref)}"`);
    attrParts.push(`xsi:type="${xsiType}"`);
    if (c.source)
        attrParts.push(`source="${esc(c.source)}"`);
    if (c.target)
        attrParts.push(`target="${esc(c.target)}"`);
    const inner = [];
    if (c.name)
        inner.push(langText(c.name, `${indent}  `, "name"));
...
function renderView(v, indent) {
    const attrParts = [`identifier="${esc(v.uuid)}"`];
    if (v.primary_viewpoint)
        attrParts.push(`viewpoint="${esc(v.primary_viewpoint)}"`);
    attrParts.push(`xsi:type="Diagram"`);
    const inner = [];
    if (v.name)
        inner.push(langText(v.name, `${indent}  `, "name"));
    if (v.desc)
        inner.push(`${indent}  <documentation xml:lang="fr">${esc(v.desc)}</documentation>`);
    for (const n of v.nodes)
        inner.push(renderNode(n, `${indent}  `));
    for (const c of v.conns)
        inner.push(renderConnection(c, `${indent}  `));
    if (inner.length === 0)
...
function renderRawNode(name, node, indent) {
    ...
    for (const [k, v] of Object.entries(n)) {
        ...
        if (k === "#text")
            continue;
        for (const child of ensureArr(v)) {
...
function renderPreservedSection(raw, sectionName, indent) {
    if (!raw)
        return "";
    const node = raw[sectionName];
    if (!node)
        return "";
    const items = ensureArr(node);
    return items.map((item) => renderRawNode(sectionName, item, indent)).join("\n");
}
// ---------------------------------------------------------------------------
// Main serializer
// ---------------------------------------------------------------------------
export function serializeToOpenExchange(model) {
    ...
    if (model.version)
        modelAttrs.push(`version="${esc(model.version)}"`);
    out.push(`<model ${modelAttrs.join(" ")}>`);
    if (model.name)
        out.push(langText(model.name, "  ", "name"));
    if (model.desc)
        out.push(`  <documentation xml:lang="fr">${esc(model.desc)}</documentation>`);
    ...
    if (model.views.length > 0 || (raw && raw["views"])) {
        ...
        out.push("  <views>");
        if (vpNode) {
            for (const vp of ensureArr(vpNode)) {
...
export function saveModelToFile(model, filePath) {
