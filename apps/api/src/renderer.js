...
function _typeToKebab(type) {
    return type.replace(/([A-Z])/g, (m, _, i) => (i > 0 ? `-${m.toLowerCase()}` : m.toLowerCase()));
}
function loadIconDataUri(type) {
    if (_iconCache.has(type))
        return _iconCache.get(type) ?? null;
...
const DEFAULT_REL_LINE = { dashArray: null, markerStart: null, markerEnd: "url(#arrow-open)" };
function collectNodes(nodes, _offsetX, _offsetY, depth, out) {
...
function derivedLineColor(fill) {
    let r = 0, g = 0, b = 0;
...
function rectEdge(fx, fy, rx, ry, rw, rh) {
    const cx = rx + rw / 2;
    const cy = ry + rh / 2;
    const dx = cx - fx;
    const dy = cy - fy;
...
function _orthogonalRoute(sx, sy, sw, sh, tx, ty, tw, th) {
    const scy = sy + sh / 2;
    const scx = sx + sw / 2;
    const tcx = tx + tw / 2;
    const tcy = ty + th / 2;
...
function getShapeKind(type, archiType = 0) {
    if (type === "AndJunction" || type === "OrJunction")
        return "junction";
    if (type === "Grouping")
        return "grouping";
    if (type === "DataObject" || type === "BusinessObject" || type === "Artifact")
        return "data-object";
    if (archiType !== 1)
        return "rect";
    if (type.endsWith("Service"))
        return "service";
    if (type.endsWith("Event") || type === "ImplementationEvent")
        return "event";
    if (type === "ApplicationComponent")
        return "component";
    if (type.endsWith("Collaboration"))
        return "collaboration";
    if (type.endsWith("Interface"))
        return "interface";
    if (type.endsWith("Function"))
        return "function";
    if (type.endsWith("Process"))
        return "process";
...
function shapeRect(x, y, w, h, fill, stroke, lw, rx = 2) {
    const svg = [
...
function shapeGrouping(x, y, w, h, fill, stroke, lw, _FONT) {
    return {
        svg: [
...
function shapeService(x, y, w, h, fill, stroke, lw) {
...
function shapeEvent(x, y, w, h, fill, stroke, lw) {
...
function shapeComponent(x, y, w, h, fill, stroke, lw) {
...
function shapeCollaboration(x, y, w, h, fill, stroke, lw) {
...
function shapeInterface(x, y, w, h, fill, stroke, lw) {
...
function shapeFunction(x, y, w, h, fill, stroke, lw) {
...
function shapeProcess(x, y, w, h, fill, stroke, lw) {
    const x1 = x + w * 0.7;
    const y1 = y + h / 5;
    const y2 = y + h - h / 5;
    const d = `M ${x},${y1} ` +
...
function shapeInteraction(x, y, w, h, fill, stroke, lw) {
    const FRACTION = 0.86;
    let diameter;
    let x1;
    let x2;
...
function shapeDataObject(x, y, w, h, fill, stroke, lw) {
    const svg = [
...
// ---------------------------------------------------------------------------
export function renderViewToSvg(view, model) {
    const PADDING = 10;
    const FONT = "Arial,Helvetica,sans-serif";
    // Build relationship lookup (id → type, name)
    const relTypeMap = new Map(model.relationships.map((r) => [r.uuid, r.type]));
    const relNameMap = new Map(model.relationships.map((r) => [r.uuid, r.name]));
    ...
    const visualChildPairs = new Set();
    function collectChildPairs(nodes, parentId) {
        for (const n of nodes) {
            if (parentId)
                visualChildPairs.add(`${parentId}→${n.uuid}`);
    ...
    for (const g of geomList) {
        ...
        if (kind === "grouping") {
            ...
            if (groupingIcon) {
                out.push(`<image href="${groupingIcon}" x="${(x + absW - ICON_SIZE - ICON_PAD).toFixed(1)}" ` +
                    `y="${(y + ICON_PAD).toFixed(1)}" width="${ICON_SIZE}" height="${ICON_SIZE}"/>`);
            }
            if (name) {
                out.push(`<text x="${x + 5}" y="${nameY}" font-family="${FONT}" font-size="11" ` +
        ...
        switch (kind) {
            case "service":
                shape = shapeService(x, y, absW, absH, fill, stroke, lw);
                break;
            case "event":
                shape = shapeEvent(x, y, absW, absH, fill, stroke, lw);
                break;
            case "component":
                shape = shapeComponent(x, y, absW, absH, fill, stroke, lw);
                break;
            case "collaboration":
                shape = shapeCollaboration(x, y, absW, absH, fill, stroke, lw);
                break;
            case "interface":
                shape = shapeInterface(x, y, absW, absH, fill, stroke, lw);
                break;
            case "function":
                shape = shapeFunction(x, y, absW, absH, fill, stroke, lw);
                break;
            case "process":
                shape = shapeProcess(x, y, absW, absH, fill, stroke, lw);
                break;
            case "interaction":
                shape = shapeInteraction(x, y, absW, absH, fill, stroke, lw);
                break;
            case "data-object":
                shape = shapeDataObject(x, y, absW, absH, fill, stroke, lw);
                break;
            default:
                shape = shapeRect(x, y, absW, absH, fill, stroke, lw);
                break;
        }
        out.push(...shape.svg);
        ...
        if (iconUri && shape.iconX !== null && shape.iconY !== null) {
            out.push(`<image href="${iconUri}" x="${shape.iconX.toFixed(1)}" y="${shape.iconY.toFixed(1)}" ` +
        ...
        if (name) {
            ...
            for (let i = 0; i < lines.length; i++) {
                out.push(`<text x="${shape.textX.toFixed(1)}" y="${(startY + i * lineH).toFixed(1)}" ` +
    ...
    for (const conn of view.conns) {
        ...
        if (NESTED_SUPPRESSED.has(relType) &&
        ...
        out.push(`<polyline points="${pointsAttr}" fill="none" ` +
        ...
        if (label) {
            ...
            out.push(`<text x="${mx.toFixed(1)}" y="${my.toFixed(1)}" font-family="${FONT}" font-size="9" ` +
...
// ---------------------------------------------------------------------------
export async function renderViewToPng(view, model) {
    let sharpFn;
    try {
