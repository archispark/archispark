 ...
 * packages/db/seeds/demo-orgs.json) with real ids/Keycloak subs.
...
function relationshipLines(r: ArchiModel["relationships"][number]): string[] {
  ...
  for (const [defUuid, val] of Object.entries(r.props)) {
    lines.push(
      `    INSERT INTO relationship_properties (relationship_id, property_def_uuid, value)`
    )
...
function connectionLines(
  c: ArchiModel["views"][number]["conns"][number]
): string[] {
  const lines = [
...
function viewLines(v: ArchiModel["views"][number]): string[] {
  ...
  for (const nd of flat) {
    ...
    lines.push(
      `      VALUES (view_db_id, ${q(nd.uuid)}, ${q(nd.elementUuid)}, ${q(nd.parentNodeUuid)}, ${q(nd.name)}, ${n(nd.x)}, ${n(nd.y)}, ${n(nd.w)}, ${n(nd.h)}, ${n(nd.fillColorR)}, ${n(nd.fillColorG)}, ${n(nd.fillColorB)}, ${n(nd.fillColorA)}, ${n(nd.lineColorR)}, ${n(nd.lineColorG)}, ${n(nd.lineColorB)}, ${n(nd.lineColorA)}, ${q(nd.fontName)}, ${n(nd.fontSize)}, ${n(nd.fontColorR)}, ${n(nd.fontColorG)}, ${n(nd.fontColorB)}, ${n(nd.fontColorA)}, ${n(nd.lineWidth)}, ${nd.sortOrder})`
    )
...
function workspaceBlock(model: ArchiModel): string {
  ...
  lines.push(``)
  lines.push(
    `  DELETE FROM workspaces WHERE organization_id = v_organization_id AND name = ${q(ws)};`
  )
  lines.push(
    `  INSERT INTO workspaces (uuid, name, description, version, organization_id, created_by_id, created_at, updated_at)`
  )
  lines.push(
    `    VALUES (${q(model.uuid)}, ${q(ws)}, ${q(model.desc)}, ${q(model.version)}, v_organization_id, v_created_by_id, EXTRACT(EPOCH FROM NOW())::INT, EXTRACT(EPOCH FROM NOW())::INT)`
  )
