...
async function adminGet<T>(path: string): Promise<T> {
  ...
  if (!res.ok) {
    throw new Error(`Keycloak admin request failed: GET ${path} -> ${res.status}`);
  }
  return (await res.json()) as T;
}

/** POSTs a JSON body and returns the id of the created resource (last path segment of the Location header). */
async function adminPostForLocation(path: string, body: unknown): Promise<string> {
  const res = await adminRequest(path, {
    method: "POST",
  ...
  if (!res.ok) {
    throw new Error(`Keycloak admin request failed: POST ${path} -> ${res.status}`);
  }
  const location = res.headers.get("location");
  if (!location) {
    throw new Error(`Keycloak admin response missing Location header for POST ${path}`);
  }
...
async function adminPut(path: string, body?: unknown): Promise<void> {
  ...
  if (!res.ok) {
    throw new Error(`Keycloak admin request failed: PUT ${path} -> ${res.status}`);
  }
}
...
async function adminDeleteWithBody(path: string, body: unknown): Promise<void> {
  ...
  if (!res.ok) {
    throw new Error(`Keycloak admin request failed: DELETE ${path} -> ${res.status}`);
  }
}

/** Lists all users in the realm (requires `query-users`). */
export function listRealmUsers(): Promise<KeycloakUserRepresentation[]> {
  return adminGet<KeycloakUserRepresentation[]>("/users?max=1000");
}

/** Returns the user with this exact username, or null if none exists. */
export async function findUserByUsername(username: string): Promise<KeycloakUserRepresentation | null> {
  const users = await adminGet<KeycloakUserRepresentation[]>(`/users?username=${encodeURIComponent(username)}&exact=true`);
  return users.find((u) => u.username === username) ?? null;
}

/** Returns the user with this id, or null if it doesn't exist (404). */
export async function getKeycloakUser(userId: string): Promise<KeycloakUserRepresentation | null> {
  const res = await adminRequest(`/users/${userId}`);
  if (res.status === 404) return null;
  if (!res.ok) {
    throw new Error(`Keycloak admin request failed: GET /users/${userId} -> ${res.status}`);
  }
...
/** Deletes a user. Returns false if the user didn't exist (404). */
export async function deleteKeycloakUser(userId: string): Promise<boolean> {
  ...
  if (!res.ok) {
    throw new Error(`Keycloak admin request failed: DELETE /users/${userId} -> ${res.status}`);
  }
  return true;
}

export function setUserPassword(userId: string, password: string, temporary = false): Promise<void> {
  return adminPut(`/users/${userId}/reset-password`, { type: "password", value: password, temporary });
}

export function getUserRealmRoles(userId: string): Promise<KeycloakRoleRepresentation[]> {
  return adminGet<KeycloakRoleRepresentation[]>(`/users/${userId}/role-mappings/realm`);
}
...
export async function unassignRealmRole(userId: string, roleName: string): Promise<void> {
  const role = await getRealmRole(roleName);
  await adminDeleteWithBody(`/users/${userId}/role-mappings/realm`, [role]);
}

/** Lists the users that have this realm role assigned. */
export function listRealmRoleUsers(roleName: string): Promise<KeycloakUserRepresentation[]> {
  return adminGet<KeycloakUserRepresentation[]>(`/roles/${roleName}/users`);
}
