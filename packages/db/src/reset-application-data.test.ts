...
describe("truncateApplicationTables", () => {
  it("wipes application data but preserves migration history", async () => {
    await db.insert(organizations).values({ slug: "to-be-wiped", name: "To be wiped" })
    await seedLocalUsers([
      { username: "visitor", email: "visitor@archispark.internal", password: "visitor" },
    ])
    await expect(
      db.select().from(organizations).where(eq(organizations.slug, "to-be-wiped"))
