 ...
 *
 * With no env file argument, loads `.env.$ENV` from the repo root (`.env.dev`
 * by default, same default as the root `pnpm env`/`pnpm up` scripts) if
...
if (!process.env["DATABASE_URL"]) {
  console.error("Missing DATABASE_URL. Pass an env file as second argument or set the variable.");
  process.exit(1);
}
...
const [ws] = await db.select().from(workspaces).where(eq(workspaces.uuid, workspaceUuid));
if (!ws) {
